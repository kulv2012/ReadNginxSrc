    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    us->peer.init = ngx_http_upstream_init_round_robin_peer;//设置一个请求的初始化回调。
...
{//ngx_http_upstream_init_request准备好FCGI数据，buffer后，会调用这里进行一个peer的初始化，此处是轮询peer的初始化。
    ngx_uint_t                         n;
