...
ngx_log_stderr(ngx_err_t err, const char *fmt, ...)
{
    ...
    va_end(args);
    if (err) {
        p = ngx_log_errno(p, last, err);//追加错误信息
    }
    if (p > last - NGX_LINEFEED_SIZE) {
        p = last - NGX_LINEFEED_SIZE;
    }
...
#endif
    buf = ngx_strerror_r(err, buf, last - buf);// 返回尾部
    if (buf < last) {
        *buf++ = ')';
    }
    return buf;
...
//kulv:not in full path 
    if (name[0] != '/') {
        ...
        if (plen) {
            ...
            p = ngx_cpymem(name, prefix, plen);//k 先拷贝路径前缀
            if (!ngx_path_separator(*(p - 1))) {
                *p++ = '/';
            }
            ngx_cpystrn(p, (u_char *) NGX_ERROR_LOG_PATH, nlen + 1);//k 拷贝日志名
            p = name;//k 还是指向开头吧
        }
    }
//k  open((const char *) name, mode|create, access),就简单打开这个文件
