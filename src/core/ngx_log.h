...
struct ngx_log_s {
    ...
    ngx_open_file_t     *file;//k fd,filename,buf的包装

    ngx_atomic_uint_t    connection;//k 干嘛用的

    ngx_log_handler_pt   handler;//错误日志打印的句柄,用于给不同的连接，等打印特定的日志结构
    void                *data;//可能设置为ngx_http_log_ctx_t，就是对应的请求类型，有特定的这个字段
    ...
    char                *action;//这个日志结构所代表的操作是啥，一般指向静态的地址
};
