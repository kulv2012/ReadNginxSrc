...
{//ngx_http_process_requestµ÷ÓÃÕâÀï£¬½øÈë´¦Àí¹ý³ÌµÄ½âÎö£¬ÖØ¶¨ÏòµÈ¡£´ËÊ±HEADERÒÑ¾­¶ÁÈ¡Íê±Ï¡£
//	ngx_http_process_requestºÍngx_http_internal_redirectµ÷ÓÃÕâÀï¡£

    ngx_http_core_main_conf_t  *cmcf;
    r->connection->log->action = NULL;
    r->connection->unexpected_eof = 0;
    if (!r->internal) {//¸Õ¿ªÊ¼internalÎª0£¬±íÊ¾²»ÊÇÔÚÄÚ²¿ÖØ¶¨Ïò¹ý³ÌÖÐ¡£
        switch (r->headers_in.connection_type) {////Á¬½ÓÊÇkeep-alive»¹ÊÇÒªclose
        case 0://Ã»ÓÐÉèÖÃ¡£Ôò½øÐÐÄ¬ÈÏ¡£ÐèÒª¿´ÊÇ·ñÊÇHTTP 1.1¡£ HTTP 1.0²»Ö§³Ökeepalive
        ...
        if (r->keepalive) {//Èç¹ûÒª±£³ÖÁ¬½Ó¡£ÈçÏÂ¼¸ÖÖÇé¿ö²»Ö§³Ö£¬±ÈÈçIE6µÄPOST£¬safariä¯ÀÀÆ÷¡£
        ...
        if (r->headers_in.content_length_n > 0) {
            r->lingering_close = 1;
//lingering_closeÓÃÓÚÔÚÇëÇóÍê³Éºó£¬½ÓÊÕ¿Í»§¶Ë·¢ËÍµÄ¶îÍâÊý¾Ý²¢È«²¿ºöÂÔµô£¬·ÀÖ¹Òò¹Ø±ÕÁ¬½ÓÊ±½ÓÊÕ»º³åÇøÖÐÈÔÓÐÊý¾Ý¶øÍ£Ö¹·¢ËÍÏìÓ¦²¢·µ»ØRST¡£
        } else {
            r->lingering_close = 0;
        }
        r->phase_handler = 0;//ÒòÎª²»ÊÇÔÚÄÚ²¿ÖØ¶¨ÏòÑ­»·ÀïÃæ£¬Òò´Ë¿Ï¶¨ÊÇµÚÒ»´Î½øÈë£¬ÄÇ¾Í´ÓµÚÒ»¸ö½×¶Î¿ªÊ¼´¦Àí°É¡£
    } else {
    //Îª1±íÊ¾:ÉèÖÃÇëÇóÎªÄÚ²¿ÖØ¶¨Ïò×´Ì¬¡£Í¨Öªngx_http_handler,½øÐÐ¼ä¶ÏÑ¡ÔñµÄÊ±ºò´Óserver_rewrite_index¿ªÊ¼½øÐÐÑ­»·´¦Àí£¬²»È»ÓÖ»ØÈ¥ÁË
        cmcf = ngx_http_get_module_main_conf(r, ngx_http_core_module);
        r->phase_handler = cmcf->phase_engine.server_rewrite_index;//»ñÈ¡rewrite¹ý³ÌµÄÏÂ±ê£¬´ÓÄÇÀï¿ªÊ¼½âÎö¡£
    }
    r->valid_location = 1;
#if (NGX_HTTP_GZIP)
    r->gzip_tested = 0;
    ...
    r->gzip_vary = 0;
#endif
    r->write_event_handler = ngx_http_core_run_phases;//ÇëÇóµÄ¿ÉÐ´ÊÂ¼þ´¦ÀíÉèÖÃÎªÕâ¸ö£¬ÕâÑù¾Íµ÷ÆðÁË¹ý³Ì´¦Àí¡£
    ngx_http_core_run_phases(r);
}

/* ÉÏÃæµÄ¸÷¸ö½×¶ÎÄ¬ÈÏÇé¿öÏÂµÄ¾ä±ú·Ö±ðÎª:ngx_http_init_phase_handlersÀïÃæÉèÖÃµÄ
...
8 {checker = 0x42f889 <ngx_http_core_access_phase>, handler = 0x45711a <ngx_http_access_handler>, next = 11}//¼òµ¥µÄIP½ûÖ¹
9 {checker = 0x42f889 <ngx_http_core_access_phase>, handler = 0x456b81 <ngx_http_auth_basic_handler>, next = 11}//¼òµ¥HTTPÃÜÂëÑéÖ¤
...
void ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    while (ph[r->phase_handler].checker) {//Ò»¸ö¸ö¹ý³ÌÍùºóÃæ½øÐÐ´¦Àí
        rc = ph[r->phase_handler].checker(r, &ph[r->phase_handler]);//ÀïÃæ»á¸Ä±är->phase_handlerµÄ¡£Ò²¾ÍÊÇid»á±ä»¯£¬Ôö¼Ó
        if (rc == NGX_OK) {//Èç¹û·µ»ØOK£¬ÔòËµÃ÷ÇëÇóÍê±Ï£¬·µ»Ø
            return;
        }
    }
}
...
{//µÚÒ»²½ºÍ×îºóÒ»²½µ÷ÓÃ¡£¼ûÉÏÃæµÄÊý×éÁÐ±í¡£
    ngx_int_t  rc;
    /*
	...
	//ngx_http_realip_handler »òÕßngx_http_log_handler 
    rc = ph->handler(r);
    if (rc == NGX_OK) {
...
ngx_http_core_rewrite_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
	...
	//µ÷ÓÃngx_http_rewrite_handler
    rc = ph->handler(r);

    if (rc == NGX_DECLINED) {//ºóÃæµÄ´¦Àí¹ý³Ì»¹Òª¼ÌÐø´¦Àí¡£
        r->phase_handler++;
        return NGX_AGAIN;
    }

    if (rc == NGX_DONE) {//´¦ÀíÍê³É£¬Ôò½«ÍÆ³ö´¦Àí¹ý³ÌµÄ½âÎö¡£
        return NGX_OK;
    }
 ...
 */
    rc = ngx_http_core_find_location(r);//½âÎöÍêHTTP{}¿éºó£¬ngx_http_init_static_location_treesº¯Êý»á´´½¨Ò»¿ÅinclusiveµÄÈý²æÊ÷£¬ÒÔ¼ÓËÙÅäÖÃ²éÕÒ¡£
    ...
    if (r->headers_in.content_length_n != -1 && !r->discard_body //³¤¶È³¬³¤ÁË¡£¾Ü¾ø
    ...
    if (rc == NGX_DONE) {//auto redirect,ÐèÒª½øÐÐÖØ¶¨Ïò¡£ÏÂÃæ¾Í¸ø¿Í»§¶Ë·µ»Ø301£¬´øÉÏÕýÈ·µÄlocationÍ·²¿
        r->headers_out.location = ngx_list_push(&r->headers_out.headers);//Ôö¼ÓÒ»¸ölocationÍ·
        ...
        if (r->args.len == 0) {//Èç¹û¿Í»§¶ËÇëÇóÃ»ÓÃ´ø²ÎÊý£¬±ÈÈçÇëÇóÊÇ: GET /AAA/BBB/
            ...
            len = clcf->name.len + 1 + r->args.len;//¼ÆËã301µÄÇ°ÃæÐ´ËÀµÄ³¤¶È£¬+²ÎÊý³¤¶È£¬ÏÂÃæÉêÇëÄÇÃ´´óµÄ¿Õ¼ä¡£
            p = ngx_pnalloc(r->pool, len);
            if (p == NULL) {
                ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
                return NGX_OK;
            }
            r->headers_out.location->value.len = len;//×Ü³¤¶È
            r->headers_out.location->value.data = p;//¼ÇÂ¼×Ö·û´®µØÖ·
            p = ngx_cpymem(p, clcf->name.data, clcf->name.len);//¿½±´Êý¾Ý
            *p++ = '?';
            ngx_memcpy(p, r->args.data, r->args.len);//²ÎÊýµÃ¿½±´ÁË£¬¸ø¿Í»§¶Ë´«µÝ¡£
        }
		//µ½ÕâÀï£¬½áÊøÕâ¸öÁ´½Ó¡£·µ»Ø301ÓÀ¾ÃÖØ¶¨Ïò¡£
...
{//ÅÐ¶ÏÒ»ÏÂÊÇ·ñÄÚ²¿ÖØ¶¨Ïò³¬¹ý11´Î¡£Ã»×öÆäËûÊÂÇé¡£
     ...
     */
    r->uri_changes--;
    if (r->uri_changes == 0) {//ÖØ¶¨Ïò³¬¹ý11´ÎÁË£¬ÖÐ¶ÏÇëÇó¡£
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "rewrite or internal redirection cycle while processing \"%V\"", &r->uri);
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return NGX_OK;
    }

    r->phase_handler = ph->next;//½«ÆäÉèÖÃÎªÏÂÒ»¸ö´¦Àí¾ä±ú£¬ÆäÊµÊÇÅäÖÃ²éÕÒÏà¹ØµÄ¡£checkerÎªngx_http_core_find_config_phase
...
{//·µ»ØNGX_OK½«ÖÕÖ¹ºóÐøµÄ¹ý³Ì´¦Àí¡£
    ngx_int_t                  rc;
    ngx_http_core_loc_conf_t  *clcf;

    if (r != r->main) {//Èç¹û²»ÊÇÖ÷ÇëÇó£¬Ôò²»ÓÃÌØÊâ´¦ÀíÖ±½Ó×ªµ½ÏÂÒ»²½¡£
        r->phase_handler = ph->next;
        return NGX_AGAIN;
    }
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,  "access phase: %ui", r->phase_handler);
    rc = ph->handler(r);//ngx_http_access_handler ºÍngx_http_auth_basic_handlerµÈ
    if (rc == NGX_DECLINED) {
        r->phase_handler++;
        return NGX_AGAIN;
    }
    ...
    if (rc == NGX_AGAIN || rc == NGX_DONE) {
        return NGX_OK;//Èç¹ûhandler·µ»ØAGAIN»òÕßDONE£¬¾Í·µ»ØNGX_OK,´Ó¶øÖÕÖ¹ºóÐø¹ý³ÌµÄ´¦Àí¡£AGAINÔõÃ´Ò²·µ»ØOKÄØ
    }

    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);

    if (clcf->satisfy == NGX_HTTP_SATISFY_ALL) {//°²È«ÅäÖÃÖÐ£¬ÔËÐíËùÓÐµÄ£¬ÄÇÈÝÒ×ÁË¡£·µ»Ø¼ÌÐø´¦ÀíÏÂÒ»¸ö¡£
        if (rc == NGX_OK) {
            r->phase_handler++;
            return NGX_AGAIN;
        }
        ...
        if (rc == NGX_OK) {
            r->access_code = 0;//access_code Õâ¸öÖ÷ÒªÊÇ´«µÝ¸øÏÂÒ»¸öphase£¬post_access phase½øÐÐ´¦ÀíµÄ¡£ 
...
ngx_http_core_post_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph){
    ...
    r->phase_handler++;//È¥´¦ÀíÏÂÒ»¸ö¹ý³Ì»Øµ÷¡£
    return NGX_AGAIN;
}
...
//Èç¹ûÓÐcontent_handler,¾ÍÖ±½Óµ÷ÓÃ¾ÍÐÐÁË.±ÈÈçÈç¹ûÊÇFCGI£¬ÔÚÓöµ½ÅäÖÃfastcgi_pass   127.0.0.1:8777;µÄÊ±ºò
//»áµ÷ÓÃngx_http_fastcgi_passº¯Êý£¬×¢²á±¾locationµÄ´¦ÀíhanderÎªngx_http_fastcgi_handler¡£
//´Ó¶øÔÚngx_http_update_location_configÀïÃæ»á¸üÐÂcontent_handlerÖ¸ÕëÎªµ±Ç°locËù¶ÔÓ¦µÄÖ¸Õë¡£
    ...
    if (r->uri.data[r->uri.len - 1] == '/') {//Èç¹ûºóÃæÊÇÒ»¸ö/£¬ÄÇÃ´½øÐÐÖØ¶¨Ïòµ½Ä¬ÈÏindexÒ³Ãæ
...
{//¸üÐÂ¸÷ÖÖÅäÖÃ¡£¸ù¾Ýloc_confµÄÅäÖÃ£¬½«¶ÔÓ¦µÄËùÐèÊý¾ÝÉèÖÃµ½rÇëÇó½á¹¹ÌåÉÏÃæÈ¥¡£
//Ö÷ÒªµÄ»¹ÓÐ r->content_handler = clcf->handler;£¬ÉèÖÃ»Øµ÷¡£
    ngx_http_core_loc_conf_t  *clcf;
    ...
    if ((ngx_io.flags & NGX_IO_SENDFILE) && clcf->sendfile) {
        r->connection->sendfile = 1;//ÓÃsendfile·¢ËÍ
    } else {
    ...
    if (clcf->handler) {//Èç¹ûÅäÖÃÄ£¿éÉèÖÃÁËÆähandler£¬ÄÇÃ´½«ÆäÉèÖÃµ½content_handlerÖ¸ÕëÉÏÃæ£¬ÒÔ±¸´ý»áÔÚ´¦ÀíphaseµÄÊ±ºò£¬µ÷ÓÃÖ®¡£
//ngx_http_fastcgi_handler¾ÍÔÚÕâÀïÉèÖÃÁË£¬ÓÚÊÇÔÚngx_http_core_content_phase¾Í»áµ÷ÓÃÕâ¸öFCGIº¯Êý£¬È»ºóÇëÇó¾Í½»¸ø¶ÔÓ¦µÄ´¦ÀíÁË
//proxy Ä£¿éÎªngx_http_proxy_handler¡£
...
{//¹¦ÄÜ:ÕÒ³öÖ¸¶¨µÄURI¶ÔÓ¦µÄÄÇ¸ölocation½á¹¹£¬ÉèÖÃµ½r->loc_confÊý×éÉÏÃæ¡£
//µÚÒ»²½£¬²éÑ¯·Ç¾«È·Æ¥ÅäµÄstatic_locationsÈý²æÊ÷ÀïÃæÓÐÃ»ÓÐÆ¥Åä³É¹¦µÄ£»
//µÚ¶þ²½£¬ÕýÔòÆ¥Åä£»Í¬Ê±Èç¹ûÆ¥Åä³É¹¦ÁË£¬Ë³±ã¿´Ò»ÏÂÓÐÃ»ÓÐÇ¶Ì×µÄlocationÓÐ»ú»áµÄ¡£
//Õâ¸öº¯ÊýµÄr->loc_confÊµ¼ÊÉÏÓÐ¿ØÖÆµÝ¹éµÄ×÷ÓÃ¡£¸üÐÂºó£¬ËùÓÐµÄ»·¾³ÐÅÏ¢¶¼¸üÐÂÁË£¬
    ngx_int_t                  rc;
	...
	//ÕÒµ½¶ÔÓ¦µÄÅäÖÃlocationºó£¬½«loc_confÊý×éÊ×µØÖ·ÉèÖÃµ½r->loc_confÖ¸ÕëÉÏÃæ£¬ÕâÑù¾ÍÇÐ»»ÁËlocationÀ²¡£
    ...
    if (noregex == 0 && pclcf->regex_locations) {//ÓÃÁËÕýÔò±í´ïÊ½£¬¶øÇÒÄØ£¬regex_locationsÕýÔò±í´ïÊ½ÁÐ±íÀïÃæÓÐ»õ£¬ÄÇ¾ÍÆ¥ÅäÖ®¡£
        for (clcfp = pclcf->regex_locations; *clcfp; clcfp++) {//¶ÔÃ¿Ò»¸öÕýÔò±í´ïÊ½£¬Æ¥ÅäÖ®¡£
            ...
            if (n == NGX_OK) {
                r->loc_conf = (*clcfp)->loc_conf;//Æ¥Åä³É¹¦£¬¼ÇÂ¼¡£
                /* look up nested locations */
...
{//nodeÎªpclcf->static_locations£¬Ò²¾ÍÊÇÈý²æÊ÷¸ù
//ÕÒ³ör->uriËùÇëÇóµÄURIËù¶ÔÓ¦µÄlocation£¬×¢ÒâÕâ¸ö¿Ï¶¨ÊÇÒ»°ãµÄlocation£¬ÕýÔò£¬named£¬nonamed¶¼²»ÔÚÕâÀïÃæµÄ¡£
//ÕÒµ½¶ÔÓ¦µÄÅäÖÃlocationºó£¬½«loc_confÊý×éÊ×µØÖ·ÉèÖÃµ½r->loc_confÖ¸ÕëÉÏÃæ£¬ÕâÑù¾ÍÇÐ»»ÁËlocationÀ²¡£
    u_char     *uri;
    size_t      len, n;
    ngx_int_t   rc, rv;
    ...
    for ( ;; ) {
        ...
        n = (len <= (size_t) node->len) ? len : node->len;//È¡×îÐ¡³¤¶È
        rc = ngx_filename_cmp(uri, node->name, n);//¾ÍÊÇstrncasecmp
        if (rc != 0) {
            node = (rc < 0) ? node->left : node->right;//Èç¹û²»ÏàµÈ£¬Ôò¾ö¶¨Íù×ó×ß»¹ÊÇÍùÓÒ×ß¡£
            continue;
        }
		//µ½ÕâÀïÒ»¶¨ÊÇÇ°×ºÏàµÈÁË¡£
        if (len > (size_t) node->len) {//Ö»²»¹ý£¬URI³¤¶È¸ü³¤£¬ËùÒÔ»¹µÃ¿´¿´×Ó½ÚµãÅ¶
            if (node->inclusive) {//Èç¹û¸Ã½ÚµãÊÇinclusiveÀàÐÍµÄ·Ç¾«È·Æ¥Åä½Úµã¡£
                r->loc_conf = node->inclusive->loc_conf;//ÏÈ¼ÇÂ¼Ò»ÏÂloc_conf£¬´ý»áÒªÊÇÃ»ÓÐ¸üºÃµÄÆ¥ÅäÁË£¬Õâ¸ö¾ÍÊÇÁË¡£
                rv = NGX_AGAIN;

                node = node->tree;//È¥³¢ÊÔÒ»ÏÂ×ÓÊ÷°É£¬ËûÃÇÓÐÏàÍ¬µÄÇ°×º¡£
                uri += n;//Ìø¹ýÇ°ÃæÒÑ¾­Æ¥Åä³É¹¦µÄ¡£²¿·Ö¡£
                len -= n;
                continue;
            }
            /* exact only */
            node = node->right;//URI³¤¶È¸ü³¤£¬Ö»ÄÜÔÙ¿´¿´ÓÒ±ßµÄ½ÚµãÁË¡£
            continue;
        }

        if (len == (size_t) node->len) {//³¤¶ÈÏàµÈ¡£
            if (node->exact) {//ÓÖÊÇ¾«È·Æ¥Åä£¬ÄÇ±ØÈ»¾ÍOKÁË¡£
                r->loc_conf = node->exact->loc_conf;//ÏÂÃæ¹û¶Ï¸üÐÂÅäÖÃÎªexactËù´ú±íµÄloc_conf¡£
                return NGX_OK;
            } else {//²»ÊÇ¾«È·Æ¥Åä³É¹¦µÄ¡£
        ...
        if (len + 1 == (size_t) node->len && node->auto_redirect) {
			//URI³¤¶È+1ÕýºÃµÈÓÚ½Úµã³¤¶È£¬²¢ÇÒÄØ£¬ÓÐauto_redirect±êÖ¾£¬ÄÇ¾Í¿ÉÒÔÁË£¬
			//ÉÏ²ã»á¸ù¾Ý·µ»ØÖµ£¬¸ø¿Í»§¶Ë·µ»Ø301ÖØ¶¨ÏòµÄ¡£
...
{//×Ô¶¯¸ù¾Ýºó×ºÃû£¬Èç¹ûngx_http_core_default_types³õÊ¼»¯ÁËºó×º£¬
//ÕâÀï»á²éÕÒ×Ô¶¯Æ¥ÅäºÏÊÊµÄcontent_type£¬Èç¹û¿Í»§¶ËÃ»ÓÐ·¢ËÍµÄ»°¡£
    u_char                     c, *exten;
    ngx_str_t                 *type;
    ngx_uint_t                 i, hash;
    ngx_http_core_loc_conf_t  *clcf;

    if (r->headers_out.content_type.len) {//ÒÑ¾­ÓÐÁË£¬Ö±½Ó·µ»Ø¡£
        return NGX_OK;
    }

    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
    if (r->exten.len) {//Èç¹ûÇëÇóÓÐºó×º
        hash = 0;
        for (i = 0; i < r->exten.len; i++) {//Ñ­»·ÓÃÃ¿¸öºó×ºÃû×Ö·û¼ÆËãhash
            c = r->exten.data[i];
            if (c >= 'A' && c <= 'Z') {//±ä³ÉÐ¡Ð´¡£
            ...
            hash = ngx_hash(hash, c);//×î¼òµ¥µÄ¶ÔÃ¿¸ö×Ö·û×öhash . ((ngx_uint_t) key * 31 + c)
        }
		//¿´¿´ÊÇ·ñÓÐ´æÔÚÓ³ÉäµÄ¡£Ó³ÉäÔÚÕâÀïngx_http_core_default_types¡£
...
{//¸ù¾Ý¸´ÔÓ±í´ïÊ½»ñÈ¡ÆäÖµ£¬È»ºóÈ·¶¨ÊÇÖØ¶¨Ïò»¹ÊÇÒª·¢ËÍÏìÓ¦£¬½«Í·²¿Êý¾ÝºÍbodyÊý¾Ý·¢ËÍ¸ø¿Í»§¶Ë¡£
    ngx_int_t     rc;
    ngx_str_t     val;
    ngx_buf_t    *b;
    ngx_chain_t   out;
	...
	//²»ÊÇÖØ¶¨Ïò£¬Ö×Ã´°ì£¬Õâ¸öÊÂÒ»ÛçÊý¾Ý£¬Òªecho »ØÈ¥µÄ¡£
    ...
    if (r->method == NGX_HTTP_HEAD || (r != r->main && val.len == 0)) {
        return ngx_http_send_header(r);//¿Í»§¶ËÖ»Òªheader,ÄÇÃ´¾Í·¢ËÍhead°É
    }
	...
	//Òª·¢ËÍÊý¾Ý£¬ËùÒÔµÃÖð²½Ïà¹ØµÄ½á¹¹ÁË£¬Êä³öbuf
    b->pos = val.data;//ÒªÊä³öµÄÄÚÈÝ¡£
    b->last = val.data + val.len;
    b->memory = val.len ? 1 : 0;
    b->last_buf = (r == r->main) ? 1 : 0;
    b->last_in_chain = 1;
    out.buf = b;//¾ÍÒ»ÛçÊý¾Ý£¬Ò»¿é¾Í¹»ÁË¡£Òò´ËÕâ¸öÁ´±íÖ»ÓÐÒ»¸ö½Úµã¡£
    out.next = NULL;
    rc = ngx_http_send_header(r);//·¢ÊÓÍ·²¿Êý¾Ý¡£

    if (rc == NGX_ERROR || rc > NGX_OK || r->header_only) {
        return rc;
    }
	//BODY·¢ËÍº¯Êý£¬µ÷ÓÃngx_http_top_body_filterÒ»¸ö¸ö½«ËûÃÇ·¢ËÍ³öÈ¥
...
{
    if (r->err_status) {//Èç¹ûÓÐ´íÎó·¢Éú£¬Ôò·¢ËÍ´íÎó×´Ì¬¡£
        r->headers_out.status = r->err_status;
        r->headers_out.status_line.len = 0;
    }
	//µÚÒ»¸öÊÇngx_http_not_modified_header_filter£¬ºóÃæÒ»¸ö¸öÍùºóµôÓÃ¡£Ã¿¸öº¯Êý¶ÀÁ¢Ò»¸öÎÄ¼þ£¬ÄÇÀïÃæÓÐ¸ö¾²Ì¬±äÁ¿¼ÇÂ¼ËüµÄÉÏÒ»¸öfilterÊÇË­¡£
    return ngx_http_top_header_filter(r);//×îºóÒ»¸öÊÇngx_http_header_filter_moduleÄ£¿éÀïÃæµÄ·¢ËÍÄ£¿é
}
...
{//BODY·¢ËÍº¯Êý£¬µ÷ÓÃngx_http_top_body_filterÒ»¸ö¸ö½«ËûÃÇ·¢ËÍ³öÈ¥
//²ÎÊýinÎªu->out_bufs£¬Ò²¾ÍÊÇ´ý·¢ËÍµÄÊý¾Ý£¬±¾º¯ÊýÍê³Éºó£¬u->out_bufsµÄÊý¾Ý½«ÒÆ¶¯µ½busy_bufs
    ngx_int_t          rc;
    ngx_connection_t  *c;

    c = r->connection;
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0,  "http output filter \"%V?%V\"", &r->uri, &r->args);
    rc = ngx_http_top_body_filter(r, in);//Ò»¸ö¸öbody filterµ÷ÓÃ£¬×îÖÕ·¢ËÍ³öÊý¾Ý¡£µÚÒ»¸öÊÇngx_http_range_body_filter
    //×îºóÒ»¸öbody filter ÊÇngx_http_write_filter
    if (rc == NGX_ERROR) {
    ...
    ngx_str_t *uri, ngx_str_t *args)
{
    ...
    r->uri_changes--;//³õÊ¼ÉèÖÃÎªNGX_HTTP_MAX_URI_CHANGES£¬¼´×î¶àÖØ¶¨Ïò10´Ë£¬ÕâÊÇÎªÁË±ÜÃâÑ­»·ÖØ¶¨ÏòµÄÎÊÌâ¡£
    if (r->uri_changes == 0) {//
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
...
ngx_http_cleanup_add(ngx_http_request_t *r, size_t size)
{
    ...
    cln->handler = NULL;
    cln->next = r->cleanup;//»·ÐÎµÄ£¬¸ÉÂïµÄÄØ
    r->cleanup = cln;
...
/*
cf->ctx Ö¸Ïòcycle->conf_ctx[http_module],Ò²¾ÍÊÇÖ¸Ïòngx_http_conf_ctx_tµÄ½á¹¹¡£
dummyÊµ¼ÊÉÏÖ¸ÏòÎªcf->ctx->main_conf£¬Ò²¾ÍÊÇHTTPµÄmain_confÖ¸Õë
*/
static char * ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{//½âÎöµ½server{}µÄÊ±ºòµ÷ÓÃÕâÀï¡£
    char                        *rv;
    void                        *mconf;
    ngx_uint_t                   i;
    ngx_conf_t                   pcf;
    ...
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));//ÉêÇëÒ»¸öctx½á¹¹
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }
    http_ctx = cf->ctx;//Õâ2¾äÊÇÎªÁË½«server²¿·ÖµÄmain_conf¸úhttpµÄ¹²Ïí·ÅÒ»·Ý¡£
    ...
    for (i = 0; ngx_modules[i]; i++) {//ÀÏ¹æ¾Ø£¬¸÷ÖÖ³õÊ¼»¯Ò»ÏÂ¡¢¡£
        ...
        if (module->create_loc_conf) {//´´½¨locÅäÖÃ½á¹¹¡£Ã¿¸öserver¶¼ÓÐÒ»¸ö·ÝÕâÑùµÄÊý×é
    ...
    cscf->ctx = ctx;//»ØÖ¸ÎÒËùÊôÓÚµÄÕâ¸öÄ£¿éÉÏÏÂÎÄctx£¬ÄÇÃ´´ÓÕâÀï£¬Ö»ÒªÖªµÀÁËcscf£¬Ò²¾ÍÄÜ¹»ÖªµÀctxÆäËùÊôµÄctx¡£
    cmcf = ctx->main_conf[ngx_http_core_module.ctx_index];//main_confÆäÊµÊÇhttpµÄ£¬¹²ÏíÁËÒ»·Ý
    cscfp = ngx_array_push(&cmcf->servers);//µÃµ½main_confµÄcoreºËÐÄÅäÖÃ£¬ÆäÊµ¾ÍÊÇHTTPµÄserversÅäÖÃ¡£ÐÂ½¨Ò»¸öÔªËØ£¬ÓÃÀ´±£´æµ±Ç°µÄcscf¡£
    if (cscfp == NULL) {//cmcf->serversµÄÊýÁ¿²»Ò»¶¨µÈÓÚserver_nameµÄÊýÁ¿£¬ÒòÎª¿ÉÄÜÒ»¸öserverÓÐ2¸öÃû³Æ¡£
        return NGX_CONF_ERROR;//gx_http_core_srv_conf_tµÄserver_names³ÉÔ±²Å¼ÇÂ¼ÁËµ±Ç°serverµÄËùÓÐÓòÃûêÇ³Æ
    }
    *cscfp = cscf;//½«Õâ¸öcscfµÄºËÐÄsrv_confÔö¼Óµ½HTTPµÄmainÅäÖÃµÄserversÊý×éÀïÃæ£¬Ò²¾ÍµÇ¼ÇÁËÒ»ÏÂ:Ôö¼ÓÁËÒ»¸öserver¹þ¡£

    /* parse inside server{} */
    pcf = *cf;//±¸·ÝÉÏÏÂÎÄ½á¹¹
    cf->ctx = ctx;//Ê¹ÓÃµ±Ç°µÄ£¬Ò²¾ÍÊÇserverµÄctx½âÎöÏÂÃæµÄ½á¹¹¡£
    cf->cmd_type = NGX_HTTP_SRV_CONF;
    rv = ngx_conf_parse(cf, NULL);
    *cf = pcf;//»¹Ô­ÉÏÏÂÎÄ½á¹¹¡£µ½´Ëserver{}½âÎöÍê±ÏÁË¡£

    if (rv == NGX_CONF_OK && !cscf->listen) {//Èç¹ûserver{}ÀïÃæÖ¸¶¨Àïlisten 8888;¼àÌý¶Ë¿Ú£¬ÔòÔÚ½âÎöµ½¼àÌý¶Ë¿ÚµÄÊ±ºò¾Í»áµ÷ÓÃngx_http_add_listenÉèÖÃÐÅÏ¢
		//Èç¹ûÕâ¸öserverÀïÃæÃ»ÓÐÏÔÊ¾µÄÖ¸¶¨¼àÌý¶Ë¿Ú£¬ÏÂÃæÄ¬ÈÏÉèÖÃÒ»¸ö80»òÕß8000¶Ë¿Ú¡£
...
#else
        sin->sin_port = htons((getuid() == 0) ? 80 : 8000);//rootÓÃ»§¼àÌý80£¬ÆäËûÓÃ»§Ä¬ÈÏ¼àÌý8000
#endif
		...
		//½«Õâ¸öÄ¬ÈÏµÄ¼àÌý¶Ë¿ÚÉèÖÃµ½cscfÀïÃæ£¬ÒÔ¼°httpµÄmain_confµÄportsÊý×éÀïÃæ£¬ÒÔ¼°¸÷ÖÖ½«Õâ¸ö¶Ë¿ÚÁªÏµÆðÀ´¡£
        if (ngx_http_add_listen(cf, cscf, &lsopt) != NGX_OK) {//±ÈÈçhttpÀïÃæ¼ÇÂ¼Õâ¸ö¶Ë¿Ú£¬È»ºóÕâ¸ö¶Ë¿Ú¼ÇÂ¼ÆäÍ¬¶Ë¿Ú²»Í¬µØÖ·ÁÐ±í£¬ºóÕßÔÙ¼ÇÂ¼¶ÔÓ¦µÄcscf.
            return NGX_CONF_ERROR;
        }
    }
...
//ºóÃæµÄÂß¼­¿´µ½£¬ctxÖ¸Õë±ä³ÉÁËÒ»¸öÒ°Ö¸Õë£¬Ã»ÓÐµØ·½Ë÷ÒýÁË£¬µ«ÊÇ£¬ÓÉÓÚctx->loc_conf ±»ºËÐÄÄ£¿éµÄclcf->loc_confË÷Òý£¬ËùÒÔÊµ¼ÊÊý¾ÝÊÇÄÜÕÒµ½µÄ¡£
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));//Õâ¸ö·ÖÅäµÄctx·Åµ½ÄÄÀïÈ¥ÁË?
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }
    pctx = cf->ctx;//¼Ç×ÅÉÏÒ»¸ö¶«¶«£¬´ý»á¿´¿´ËûÊÇ²»ÊÇÇ¶Ì×µÄ¡£
    ctx->main_conf = pctx->main_conf;//¼Ì³Ð¶¥²ãµÄmain,srvÅäÖÃ¡£
    ctx->srv_conf = pctx->srv_conf;
	//µ«ÊÇ¶ÔÓÚloc_conf£¬ÐèÒª×Ô¼ºÊ¹ÓÃÒ»·Ý³öÀ´¡£
    ctx->loc_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);
    if (ctx->loc_conf == NULL) {
        return NGX_CONF_ERROR;
    }

    for (i = 0; ngx_modules[i]; i++) {//±éÀúÃ¿Ò»¸öHTTPÄ£¿é£¬Èç¹ûÆäÓÐcreate_loc_conf£¬Ôòµ÷ÓÃÖ®¡£ÕâÊÇÉ¶ÒâË¼?
        if (ngx_modules[i]->type != NGX_HTTP_MODULE) {
            continue;
        }
        module = ngx_modules[i]->ctx;
        if (module->create_loc_conf) {//ÒâË¼ÊÇ:´ó¼ÒºÃ£¬ÎÒÓÖÅöµ½ÁËÒ»¸ölocation¿é£¬Âé·³´ó»ï×¼±¸Ò»ÏÂÅäÖÃÊý¾Ý£¬Èç¹ûÄãÐèÒªµÄ»°¡£
	...
	//clcf»á±»pclcf->locationsËùÖ¸Ïò¡£µ«ÊÇ¸Õ¸ÕÉêÇëµÄctxºÃÏñÃ»ÓÐµØ·½Ö¸ÏòËüÁË£¬±ä³ÉÁËÒ»¸öÒ°Ö¸Õë¡£
    clcf = ctx->loc_conf[ngx_http_core_module.ctx_index];//µÃµ½HTTPÀÏ´óµÄloc_confÅäÖÃ£¬ÆäÊµËüµÄconf¿Ï¶¨µÈÓÚ¶¥²ãµÄÁË£¬ÎªÊ²Ã´ÄØ£¬ÒòÎªËûÊÇ¶¥²ãµÄ¡£
    clcf->loc_conf = ctx->loc_conf;//ÕâÊÇÉ¶ÒâË¼£¬clcfÊÇ¸Õ²Åµ÷ÓÃcreate_loc_confÊ±·µ»ØµÄ¡£Ïàµ±ÓÚÈÃÆäÒÀÈ»µÈÓÚÉÏ²ãµÄloc_conf.¹²Ïí¼´¿É¡£
    //ÉÏÃæÊÇËµ£¬ºËÐÄÄ£¿éµÄloc_conf¾ÍÊÇ±¾Ä£¿é¡£ÔÚlocationµÄ½âÎöÖÐ£¬
    //ngx_http_core_loc_conf_t½á¹¹(¼´clcf)µÄloc_conf³ÉÔ±»á±£´æËùÓÐÆäËûmoduleµÄlocationÅäÖÃÓÃµ½µÄÊ±ºòÒýÓÃ·½±ã¡£È»ºóºËÐÄÄ£¿éclcfÕýºÃ±»locationsË÷Òý
    //Òò´ËÔÚ½âÎöÅäÖÃµÄÊ±ºò£¬Í¨¹ýlocations£¬×ÔÈ»¾ÍÄÜÕÒµ½clcf£¬½ø¶øÍ¨¹ýclcf->loc_confÕÒµ½¸ÕÉêÇëµÄctx->loc_conf¡£¶ø¶ÔÓÚmain_conf£¬Ã»ÓÃÁË¡£
    value = cf->args->elts;
    if (cf->args->nelts == 3) {//location ~ \.php$ {ÕâÖÖ´¦Àí¡£
        len = value[1].len;
        mod = value[1].data;
        name = &value[2];

        if (len == 1 && mod[0] == '=') {
            clcf->name = *name;//¼Ç×¡locationºóÃæµÄÃû×ÖÊÇÊ²Ã´
            clcf->exact_match = 1;
        } else if (len == 2 && mod[0] == '^' && mod[1] == '~') {//^~ ¿ªÍ·±íÊ¾uriÒÔÄ³¸ö³£¹æ×Ö·û´®¿ªÍ·£¬Àí½âÎªÆ¥Åä urlÂ·¾¶¼´¿É¡£
            clcf->name = *name;
            clcf->noregex = 1;
        } else if (len == 1 && mod[0] == '~') {//´óÐ¡²»Ãô¸ÐµÄÕýÔòÆ¥Åä
            if (ngx_http_core_regex_location(cf, clcf, name, 0) != NGX_OK) {
                return NGX_CONF_ERROR;//½âÎöÕýÔò±í´ïÊ½¡£±£´æµ½clcfµÄregex±äÁ¿ÀïÃæ
            }
        } else if (len == 2 && mod[0] == '~' && mod[1] == '*') {//´óÐ¡Ð´Ãô¸Ð
    ...
    } else {//ÕâÀï±íÊ¾ºóÃæµÄ=,~µÈ·ûºÅÃ»ÓÐ¸úµÚÈý¸ö²ÎÊý×Ö·û´®ÓÃ¿Õ¸ñ·Ö¿ªµÄÇé¿ö¡£
        name = &value[1];
        if (name->data[0] == '=') {
    ...
    pclcf = pctx->loc_conf[ngx_http_core_module.ctx_index];//µÃµ½ÀÏµÄHTTP×ÜcoreÄ£¿éµÄÅäÖÃ¡£»òÕßËµÉÏÒ»¸ölocation½á¹¹¡£
    if (pclcf->name.len) {//ÉÏÒ»¸ölocation½á¹¹Ò²ÓÐÃû×Ö¡£ÄÇ¾ÍËµÃ÷ÕâÀïÊÇÇ¶Ì×µÄlocation.
        /* nested location */
#if 0
        clcf->prev_location = pclcf;
#endif
        if (pclcf->exact_match) {//ÉÏÒ»¸öÊÇ¾ø¶Ô=µÈÓÚºÅÆ¥ÅäµÄ£¬ÄÇ²»ÐÐ£¬·Ç·¨µÄ¡£
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,   "location \"%V\" could not be inside the exact location \"%V\"", &clcf->name, &pclcf->name);
            return NGX_CONF_ERROR;
        }
        if (pclcf->named) {//ÉÏ²ãlocationÊÇ¸öÃüÃûµÄlocation,Ò²²»ÐÐ¡£
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,  "location \"%V\" could not be inside the named location \"%V\"", &clcf->name, &pclcf->name);
            return NGX_CONF_ERROR;
        }
        if (clcf->named) {//±¾½ÚµãÊÇ¸önamed£¬Ò²²»ÐÐ£¬‡å
...
#endif
        {//ÎÞÂÛÈçºÎ£¬¶¥²ã½Úµã±ØÐëÊÇÄÚ´æ½ÚµãµÄÇ°×º¡£ÔÚÇ¶Ì×µÄÊ±ºò¡£
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "location \"%V\" is outside location \"%V\"", &clcf->name, &pclcf->name);
            return NGX_CONF_ERROR;
        }
    }
    if (ngx_http_add_location(cf, &pclcf->locations, clcf) != NGX_OK) {//½«ÎÒÕâ¸ölocation¼ÓÈëµ½ÉÏ²ãµÄlocÀïÃæ¡£×¢ÒâÊÇÉÏ²ã¡£
        return NGX_CONF_ERROR;//½«clcf·ÅÈëpclcf->locationsµÄºóÃæ£¬Á´½Óµ½¶ÓÁÐºóÃæ
    }
    save = *cf;
    cf->ctx = ctx;//Ìæ»»ÎªÎÒ×Ô¼ºµÄctx¡£ÀïÃæÕÕÑùÓÐmain/srv/locÅäÖÃ¡£
    cf->cmd_type = NGX_HTTP_LOC_CONF;
    rv = ngx_conf_parse(cf, NULL);//½âÎö×Ó¼¯
    *cf = save;
    return rv;
}
...
{//½âÎöÕýÔò±í´ïÊ½£¬½«Æä·ÅÈëregex,name×Ö¶ÎÉÏÃæ
#if (NGX_PCRE)
...
{//µ¼ÈëºËÐÄµÄÒ»Ð©±äÁ¿£¬ÔÚ½âÎöÅäÖÃÖ®Ç°¾ÍÉèÖÃÁË
...
{//½âÎöµ½"listen"µÄÊ±ºòµ÷ÓÃ¡£
    ...
    cscf->listen = 1;//±ê¼ÇÒ»ÏÂ£¬ÕâÀïÓÐlisten¶Ë¿Ú¡£
    value = cf->args->elts;
