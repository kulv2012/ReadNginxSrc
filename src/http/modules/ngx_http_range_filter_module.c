...
typedef struct {
    ...
    ngx_array_t  ranges;//客户端要求的ranges
...
{//这是第一个BODY过滤函数
    ...
    /*下面说明，NGINX只支持整个数据都在一个BODY里面的情况也就是多个body它不好处理
    ...
    ngx_http_range_filter_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    if (!buf->last_buf) {//只对前面或者中介的buff进行
        if (buf->in_file) {//得到这坨数据的其实，结束位置。不应该是起始为0么?
        ...
        range = ctx->ranges.elts;
        for (i = 0; i < ctx->ranges.nelts; i++) {
            if (start > range[i].start || last < range[i].end) {//如果客户端要求的开始位置在buf前面，或者结束位置在buf后面，则超出了本buffer的范围
