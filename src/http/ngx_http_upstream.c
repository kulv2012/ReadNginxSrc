...
{//·¢ËÍÇëÇóÊı¾İ¸ø¿Í»§¶Ë¡£ÀïÃæ»á´¦Àíheader,body·Ö¿ª·¢ËÍµÄÇé¿öµÄ 
    ...
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
    if (!u->buffering) {//×¢ÒâbufferingÄ£Ê½²»ÄÜÓÃÓÚFCGI£¬FCGIĞ´ËÀÎª1.ÒòÎªFCGIÊÇ°üÊ½µÄ´«Êä¡£·ÇÁ÷Ê½
//bufferingÖ»nginxÊÇ·´Ó¦¸ÃÏÈbuffer ºó¶ËFCGI·¢¹ıÀ´µÄÊı¾İ£¬È»ºóÒ»´Î·¢ËÍ¸ø¿Í»§¶Ë¡£
...
//ÏÂÃæ¾ÍÊÇÒª½øĞĞºó¶ËÊı¾İ»º´æ´¦ÀíµÄ¹ı³ÌÁË¡£Ò²¾ÍÊÇÊ¹ÓÃÁËbuffering±ê¼ÇµÄÌõ¼şÏÂ
    ...
    p->output_ctx = r;
    p->tag = u->output.tag;
    p->bufs = u->conf->bufs;//ÉèÖÃbufs£¬Ëü¾ÍÊÇupstreamÖĞÉèÖÃµÄbufs
...
/*ÎªÊ²Ã´ÕâÀï»¹ÓĞ¿É¶ÁÊı¾İÄØ£¬²»ÊÇÒÑ¾­½Óµ½FCGIµÄ½áÊø°üÁËÂğ£¬ÒòÎªÖ®Ç°Ö»ÊÇ¶ÁÈ¡ÍêÁËHTTPÍ·²¿,Åöµ½\r\n\r\nºó¾ÍÍË³öÁË
   ...
   »ĞÈ»´óÎò£¬FCGIĞ­ÒéÊÇÎŞËùÎ½bufferingÁË£¬Ëü²»ÊÇÁ÷Ê½µÄÊı¾İ£¬¶øÊÇ°üÊ½µÄÒ»¸ö¸ö°ü£¬ËùÒÔ±ØÈ»²»ÄÜbuffering.
...
get the whole block from upstream.*/
            ...
            if (n > 0) {//¶ÁÁËÒ»Ğ©Êı¾İ£¬Á¢Âí½«Ëü·¢ËÍ³öÈ¥°É£¬Ò²¾ÍÊÇ·ÅÈëµ½out_bufsÁ´±íÀïÃæÈ¥£¬Ã»ÓĞÊµ¼Ê·¢ËÍµÄ£¬ÄÇÃ´£¬Ê²Ã´Ê±ºò·¢ËÍÄØ
                u->state->response_length += n;//ÔÙ´Îµ÷ÓÃinput_filter,ÕâÀïÃ»ÓĞreset u->buffer.last,ÕâÊÇÒòÎªÎÒÃÇÕâ¸öÖµ²¢Ã»ÓĞ¸üĞÂ.
...
ngx_http_upstream_process_upstream(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (c->read->timedout) {//Èç¹û³¬Ê±ÁË
        ...
        ngx_connection_error(c, NGX_ETIMEDOUT, "upstream timed out");
    } else {//ÇëÇóÃ»ÓĞ³¬Ê±£¬ÄÇÃ´¶Ôºó¶Ë£¬´¦ÀíÒ»ÏÂ¶ÁÊÂ¼ş¡£
