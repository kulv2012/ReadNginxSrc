...
//Ò»°á»áÔÚ´¦Àí¹ý³ÌµÄ»Øµ÷º¯ÊýÖÐµ÷ÓÃ£¬±ÈÈçngx_http_proxy_handler£¬ngx_http_fastcgi_handlerµÈ£¬ÓÃÀ´ÉêÇëupstream´ó½á¹¹Ìå
ngx_int_t ngx_http_upstream_create(ngx_http_request_t *r)
{//´´½¨Ò»¸öÉÏÓÎÄ£¿éÐèÒªµÄ½á¹¹£¬ÉèÖÃµ½r²ÎÊýµÄ¿Í»§¶ËÇëÇó½á¹¹ÉÏÃæÈ¥¡£
    ngx_http_upstream_t  *u;

    u = r->upstream;//ÄÃµ½Õâ¸öÇëÇóµÄupstream½á¹¹,Èç¹ûÆäcleanup³ÉÔ±·Ç¿Õ£¬¾ÍÖ´ÐÐÇåÀí¡£ÎªÉ¶¿
    if (u && u->cleanup) {
        r->main->count++;
        ngx_http_upstream_cleanup(r);//Èç¹ûÒÑ¾­ÓÐÁËupstream½á¹¹£¬¾Í¸´ÓÃ¾ÉµÄ¡£
    }

    u = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_t));//È»ºóÉêÇëÒ»¸öÐÂµÄngx_http_upstream_t
    if (u == NULL) {
        return NGX_ERROR;
    }
    r->upstream = u;
...
//ÏÂÃæº¯ÊýÒ»°ãÊÇÕâÃ´±»µ÷ÓÃµÄ: ngx_http_read_client_request_body(r, ngx_http_upstream_init);Ò²¾ÍÊÇ¶ÁÈ¡Íê¿Í»§¶ËÇëÇóµÄbodyºóµ÷ÓÃÕâÀï¡£
void ngx_http_upstream_init(ngx_http_request_t *r)
{//ngx_http_read_client_request_body¶ÁÈ¡Íê±Ï¿Í»§¶ËµÄÊý¾Ýºó£¬¾Í»áµ÷ÓÃÕâÀï½øÐÐ³õÊ¼»¯Ò»¸öupstream
    ngx_connection_t     *c;
    c = r->connection;//µÃµ½ÆäÁ¬½Ó½á¹¹¡£
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "http init upstream, client timer: %d", c->read->timer_set);

    if (c->read->timer_set) {//¶ÁÍêÁË£¬½«¶ÁÊÂ¼þ½á¹¹¶¨Ê±Æ÷É¾³ý¡£
        ngx_del_timer(c->read);
    }
    if (ngx_event_flags & NGX_USE_CLEAR_EVENT) {//Èç¹ûepollÊ¹ÓÃ±ßÔµ´¥·¢
        if (!c->write->active) {//ÒªÔö¼Ó¿ÉÐ´ÊÂ¼þÍ¨Öª£¬ÎªÉ¶?ÒòÎª´ý»á¿ÉÄÜ¾ÍÄÜÐ´ÁË
    ...
    ngx_http_upstream_init_request(r);//½øÐÐÊµÖÊµÄ³õÊ¼»¯¡£
}


static void
ngx_http_upstream_init_request(ngx_http_request_t *r)
{//ngx_http_upstream_initµ÷ÓÃÕâÀï£¬´ËÊ±¿Í»§¶Ë·¢ËÍµÄÊý¾Ý¶¼ÒÑ¾­½ÓÊÕÍê±ÏÁË¡£
    ngx_str_t                      *host;
    ngx_uint_t                      i;
    ...
    if (!u->store && !r->post_action && !u->conf->ignore_client_abort) {//ignore_client_abortºöÂÔ¿Í»§¶ËÌáÇ°¶Ï¿ªÁ¬½Ó¡£ÕâÀïÖ¸²»ºöÂÔ¿Í»§¶ËÌáÇ°¶Ï¿ª¡£
        r->read_event_handler = ngx_http_upstream_rd_check_broken_connection;//ÉèÖÃ»Øµ÷ÐèÒª¼ì²âÁ¬½ÓÊÇ·ñÓÐÎÊÌâ¡£
        r->write_event_handler = ngx_http_upstream_wr_check_broken_connection;
    }
    if (r->request_body) {//¿Í»§¶Ë·¢ËÍ¹ýÀ´µÄPOSTÊý¾Ý´æ·ÅÔÚ´Ë,ngx_http_read_client_request_body·ÅµÄ
        u->request_bufs = r->request_body->bufs;//¼ÇÂ¼¿Í»§¶Ë·¢ËÍµÄÊý¾Ý£¬ÏÂÃæÔÚcreate_requestµÄÊ±ºò¿½±´µ½·¢ËÍ»º³åÁ´½Ó±íÀïÃæµÄ¡£
    }
	//Èç¹ûÊÇFCGI¡£ÏÂÃæ×é½¨ºÃFCGIµÄ¸÷ÖÖÍ·²¿£¬°üÀ¨ÇëÇó¿ªÊ¼Í·£¬ÇëÇó²ÎÊýÍ·£¬ÇëÇóSTDINÍ·¡£´æ·ÅÔÚu->request_bufsÁ´½Ó±íÀïÃæ¡£
	//Èç¹ûÊÇProxyÄ£¿é£¬ngx_http_proxy_create_request×é¼þ·´Ïò´úÀíµÄÍ·²¿É¶µÄ,·Åµ½u->request_bufsÀïÃæ
	...
	//ÉèÖÃ¹ýÂËÄ£¿éµÄ¿ªÊ¼¹ýÂËº¯ÊýÎªwriter¡£Ò²¾ÍÊÇoutput_filter¡£ÔÚngx_output_chain±»µ÷ÓÃÒÑ½øÐÐÊý¾ÝµÄ¹ýÂË
    u->output.output_filter = ngx_chain_writer;
    u->output.filter_ctx = &u->writer;//²Î¿¼ngx_chain_writer£¬ÀïÃæ»á½«Êä³öbufÒ»¸ö¸öÁ¬½Óµ½ÕâÀï¡£

    u->writer.pool = r->pool;
    if (r->upstream_states == NULL) {//Êý×éupstream_states£¬±£ÁôupstreamµÄ×´Ì¬ÐÅÏ¢¡£
        r->upstream_states = ngx_array_create(r->pool, 1, sizeof(ngx_http_upstream_state_t));
        if (r->upstream_states == NULL) {
            ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
    } else {//Èç¹ûÒÑ¾­ÓÐÁË£¬ÐÂ¼ÓÒ»¸ö¡£
	...
	//¹ÒÔÚÇåÀí»Øµ÷º¯Êý£¬¸ÉÂïµÄÔÝ²»Çå³þ
    cln = ngx_http_cleanup_add(r, 0);//»·ÐÎÁ´±í£¬ÉêÇëÒ»¸öÐÂµÄÔªËØ¡£
    if (cln == NULL) {
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
...
/*È»ºó¾ÍÊÇÕâ¸öº¯Êý×îºËÐÄµÄ´¦Àí²¿·Ö£¬ÄÇ¾ÍÊÇ¸ù¾ÝupstreamµÄÀàÐÍÀ´½øÐÐ²»Í¬µÄ²Ù×÷£¬ÕâÀïµÄupstream¾ÍÊÇÎÒÃÇÍ¨¹ýXXX_pass´«µÝ½øÀ´µÄÖµ£¬
ÕâÀïµÄupstreamÓÐ¿ÉÄÜÏÂÃæ¼¸ÖÖÇé¿ö¡£
1 XXX_passÖÐ²»°üº¬±äÁ¿¡£
2 XXX_pass´«µÝµÄÖµ°üº¬ÁËÒ»¸ö±äÁ¿($¿ªÊ¼).ÕâÖÖÇé¿öÒ²¾ÍÊÇËµupstreamµÄurlÊÇ¶¯Ì¬±ä»¯µÄ£¬Òò´ËÐèÒªÃ¿´Î¶¼½âÎöÒ»±é.
¶øµÚ¶þÖÖÇé¿öÓÖ·ÖÎª2ÖÖ£¬Ò»ÖÖÊÇÔÚ½øÈëupstreamÖ®Ç°£¬Ò²¾ÍÊÇ upstreamÄ£¿éµÄhandlerÖ®ÖÐÒÑ¾­±»resolveµÄµØÖ·(Çë¿´ngx_http_XXX_evalº¯Êý)£¬
Ò»ÖÖÊÇÃ»ÓÐ±»resolve£¬´ËÊ±¾ÍÐèÒªupstreamÄ£¿éÀ´½øÐÐresolve¡£½ÓÏÂÀ´µÄ´úÂë¾ÍÊÇ´¦ÀíÕâ²¿·ÖµÄ¶«Î÷¡£*/
    if (u->resolved == NULL) {//ÉÏÓÎµÄIPµØÖ·ÊÇ·ñ±»½âÎö¹ý£¬ngx_http_fastcgi_handlerµ÷ÓÃngx_http_fastcgi_eval»á½âÎö¡£
        uscf = u->conf->upstream;
    } else {
        if (u->resolved->sockaddr) {//Èç¹ûµØÖ·ÒÑ¾­±»resolve¹ýÁË£¬´ËÊ±´´½¨round robin peer
        ...
        uscfp = umcf->upstreams.elts;//ËùÓÐµÄÉÏÓÎÄ£¿é
        ...
        temp.name = *host;
        ctx = ngx_resolve_start(clcf->resolver, &temp);//¿½±´Ò»ÏÂIP£¬portµÈ
        if (ctx == NULL) {
            ngx_http_upstream_finalize_request(r, u,NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
...
{//Õâ¸öÊÇ¶ÁÐ´ÊÂ¼þµÄÍ³Ò»»Øµ÷º¯Êý£¬²»¹ý×Ô¼º»á¸ù¾Ý¶Á»¹ÊÇÐ´µ÷ÓÃ¶ÔÓ¦µÄwrite_event_handlerµÈ
    ngx_connection_t     *c;
    ngx_http_request_t   *r;
...
#endif
    n = recv(c->fd, buf, 1, MSG_PEEK);//MSG_PEEKÊÔÌ½Ò»¸ö×Ô¼º£¬¿´¿´ÊÇ·ñÓÐÎÊÌâ¡£
...
{//µ÷ÓÃsocket,connectÁ¬½ÓÒ»¸öºó¶ËµÄpeer,È»ºóÉèÖÃ¶ÁÐ´ÊÂ¼þ»Øµ÷º¯Êý£¬½øÈë·¢ËÍÊý¾ÝµÄngx_http_upstream_send_requestÀïÃæ
//ÕâÀï¸ºÔðÁ¬½Óºó¶Ë·þÎñ£¬È»ºóÉèÖÃ¸÷¸ö¶ÁÐ´ÊÂ¼þ»Øµ÷¡£×îºóÈç¹ûÁ¬½Ó½¨Á¢³É¹¦£¬»áµ÷ÓÃngx_http_upstream_send_request½øÐÐÊý¾Ý·¢ËÍ¡£
    ngx_int_t          rc;
    ngx_time_t        *tp;
    ngx_connection_t  *c;
	...
	//ÏÂÃæÁ¬½ÓºóµÄÄÇÄ£¿é£¬È»ºóÉèÖÃ¶ÁÐ´»Øµ÷¡£
    rc = ngx_event_connect_peer(&u->peer);//»ñÈ¡Ò»¸öpeer£¬È»ºósocket(),connect(),add_eventÖ®×¢²áÏà¹ØÊÂ¼þ½á¹¹
    ...
    if (rc == NGX_BUSY) {//Èç¹ûÕâ¸öpeer±»ÉèÖÃÎªÃ¦Âµ×´Ì¬£¬Ôò³¢ÊÔÏÂÒ»¸ö£¬»áµÝ¹é»ØÀ´µÄ¡£
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "no live upstreams");
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_NOLIVE);//³¢ÊÔ»Ö¸´Ò»Ð©ÉÏÓÎÄ£¿é£¬È»ºóµÝ¹éµ÷ÓÃngx_http_upstream_connect½øÐÐÁ¬½Ó¡£
        return;
    }
    ...
    c = u->peer.connection;//µÃµ½Õâ¸öpeerÐÂ½¨Á¢µÄÁ¬½Ó½á¹¹¡£
    c->data = r;//¼Ç×¡ÎÒÕâ¸öÁ¬½ÓÊôÓÚÄÄ¸öÇëÇó¡£
    c->write->handler = ngx_http_upstream_handler;//ÉèÖÃÕâ¸öÁ¬½ÓµÄ¶ÁÐ´ÊÂ¼þ½á¹¹¡£ÕâÊÇÕæÕýµÄ¶ÁÐ´ÊÂ¼þ»Øµ÷¡£ÀïÃæ»áµ÷ÓÃwrite_event_handler¡£
    c->read->handler = ngx_http_upstream_handler;//Õâ¸öÊÇ¶ÁÐ´ÊÂ¼þµÄÍ³Ò»»Øµ÷º¯Êý£¬²»¹ý×Ô¼º»á¸ù¾Ý¶Á»¹ÊÇÐ´µ÷ÓÃ¶ÔÓ¦µÄwrite_event_handlerµÈ

	//Ò»¸öupstreamµÄ¶ÁÐ´»Øµ÷£¬×¨ÃÅ×ö¸úupstreamÓÐ¹ØµÄÊÂÇé¡£ÉÏÃæµÄ»ù±¾¶ÁÐ´ÊÂ¼þ»Øµ÷ngx_http_upstream_handler»áµ÷ÓÃÏÂÃæµÄº¯ÊýÍê³Éupstream¶ÔÓ¦µÄÊÂÇé¡£
	u->write_event_handler = ngx_http_upstream_send_request_handler;//ÉèÖÃÐ´ÊÂ¼þµÄ´¦Àíº¯Êý¡£
    ...
    u->writer.out = NULL;//ÕâÊÇiÓÃÀ´¸øngx_chain_writeº¯Êý¼ÇÂ¼·¢ËÍ»º³åÇøµÄ¡£
    u->writer.last = &u->writer.out;//Ö¸Ïò×Ô¼ºµÄÍ·²¿¡£ÐÎ³ÉÑ­»·µÄ½á¹¹¡£
    u->writer.connection = c;
    u->writer.limit = 0;

    if (u->request_sent) {//Èç¹ûÊÇÒÑ¾­·¢ËÍÁËÇëÇó¡£È´»¹ÐèÒªÁ¬½Ó£¬µÃÖØÐÂ³õÊ¼»¯Ò»ÏÂ
    ...
    {//request_bodyÊÇFCGI½á¹¹µÄÊý¾Ý¡£
        ...
        //Ö¸ÏòÇëÇóµÄBODYÊý¾Ý²¿·Ö£¬É¶ÒâË¼?°ÑÊý¾Ý·Åµ½output±äÁ¿ÀïÃæ£¬´ý»áµ½send_requestÀïÃæ»á¿½±´µ½Êä³öÁ´±í½øÐÐ·¢ËÍ
        u->output.free->buf = r->request_body->buf;
        u->output.free->next = NULL;
        u->output.allocated = 1;
		//Çå¿ÕÕâ¿éÄÚ´æ£¬¸ÉÂïÄØ?ÒòÎªÇëÇóµÄFCGIÊý¾ÝÒÑ¾­¿½±´µ½ÁËngx_http_upstream_sµÄrequest_bufsÁ´½Ó±íÀïÃæ
    ...
    if (rc == NGX_AGAIN) {//Èç¹û¸Õ²ÅµÄrc±íÊ¾Á¬½ÓÉÐÎ´½¨Á¢£¬ÔòÉèÖÃÁ¬½Ó³¬Ê±Ê±¼ä¡£
...
#endif
    ngx_http_upstream_send_request(r, u);//ÒÑ¾­Á¬½Ó³É¹¦ºó¶Ë£¬ÏÂÃæ½øÐÐÊý¾Ý·¢ËÍ¡£
}
...
{//µ÷ÓÃÊä³öµÄ¹ýÂËÆ÷£¬·¢ËÍÊý¾Ýµ½ºó¶Ë
    ngx_int_t          rc;
    ngx_connection_t  *c;
    c = u->peer.connection;//ÄÃµ½Õâ¸öpeerµÄÁ¬½Ó
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "http upstream send request");
	//²âÊÔÒ»¸öÁ¬½Ó×´Ì¬£¬Èç¹ûÁ¬½ÓËð»µ£¬ÔòÖØÊÔ
    if (!u->request_sent && ngx_http_upstream_test_connect(c) != NGX_OK) {
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_ERROR);
        return;
    }
    c->log->action = "sending request to upstream";
	//ÏÂÃæ¿ªÊ¼¹ýÂËÄ£¿éµÄ¹ý³Ì¡£¶ÔÇëÇóµÄFCGIÊý¾Ý½øÐÐ¹ýÂË£¬ÀïÃæ»áµ÷ÓÃngx_chain_writer£¬½«Êý¾ÝÓÃwritev·¢ËÍ³öÈ¥
	//ngx_http_upstream_connect½«¿Í»§¶Ë·¢ËÍµÄÊý¾Ý¿½±´µ½ÕâÀï£¬Èç¹ûÊÇ´Ó¶ÁÐ´ÊÂ¼þ»Øµ÷½øÈëµÄ£¬ÔòÕâÀïµÄrequest_sentÓ¦¸ÃÎª1£¬
	//±íÊ¾Êý¾ÝÒÑ¾­¿½±´µ½Êä³öÁ´ÁË¡£Õâ·ÝÊý¾ÝÊÇÔÚngx_http_upstream_init_requestÀïÃæµ÷ÓÃ´¦ÀíÄ£¿é±ÈÈçFCGIµÄcreate_request´¦ÀíµÄ£¬½âÎöÎªFCGIµÄ½á¹¹Êý¾Ý¡£
    rc = ngx_output_chain(&u->output, u->request_sent ? NULL : u->request_bufs);
    u->request_sent = 1;//±êÖ¾Î»Êý¾ÝÒÑ¾­·¢ËÍÍê±Ï,Ö¸µÄÊÇ·ÅÈëÊä³öÁÐ±íÀïÃæ£¬²»Ò»¶¨·¢ËÍ³öÈ¥ÁË¡£

    if (rc == NGX_ERROR) {//Èç¹û³ö´í£¬¼ÌÐø
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_ERROR);
        return;
    }
    if (c->write->timer_set) {//ÒÑ¾­²»ÐèÒªÐ´Êý¾ÝÁË¡£
        ngx_del_timer(c->write);
    }

    if (rc == NGX_AGAIN) {//Êý¾Ý»¹Ã»ÓÐ·¢ËÍÍê±Ï£¬´ý»á»¹ÐèÒª·¢ËÍ¡£
        ngx_add_timer(c->write, u->conf->send_timeout);
        if (ngx_handle_write_event(c->write, u->conf->send_lowat) != NGX_OK) {//×¢²áÒ»ÏÂ¶ÁÐ´ÊÂ¼þ¡£
            ngx_http_upstream_finalize_request(r, u, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
        }
        return;
    ...
    if (c->tcp_nopush == NGX_TCP_NOPUSH_SET) {
        if (ngx_tcp_push(c->fd) == NGX_ERROR) {//ÉèÖÃPUSH±êÖ¾Î»£¬¾¡¿ì·¢ËÍÊý¾Ý¡£
    ...
    ngx_add_timer(c->read, u->conf->read_timeout);//Õâ»ØÊý¾ÝÒÑ¾­·¢ËÍÁË£¬¿ÉÒÔ×¼±¸½ÓÊÕÁË£¬ÉèÖÃ½ÓÊÕ³¬Ê±¶¨Ê±Æ÷¡£
#if 1
...
#endif
    u->write_event_handler = ngx_http_upstream_dummy_handler;//²»ÓÃÐ´ÁË£¬Ö»ÐèÒª¶Á
    if (ngx_handle_write_event(c->write, 0) != NGX_OK) {
        ngx_http_upstream_finalize_request(r, u,
                                           NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
	//ÏÂÃæ£¬ÓÉÓÚÕâ¸ö¾ä±ú»¹Ã»Êý¾Ý¿ÉÒÔ¶ÁÈ¡£¬ÎÒÃÇ¿ÉÒÔÏÈ»ØÈ¥¸ÉÆäËûÊÂÇéÁË£¬ÒòÎªÔÚngx_http_upstream_connectÀïÃæÒÑ¾­ÉèÖÃÁË¶ÁÊý¾ÝµÄ»Øµ÷º¯ÊýÁËµÄ¡£
}


static void
ngx_http_upstream_send_request_handler(ngx_http_request_t *r, ngx_http_upstream_t *u)
{//Èç¹û¾ä±ú¿ÉÒÔÐ´ÁË£¬Ôòµ÷ÓÃÕâÀï×¼±¸·¢ËÍÊý¾Ýµ½ºó¶ËÈ¥¡£´Ë´¦Ö»´¦ÀíÒ»ÏÂÐ´³¬Ê±µÈ¡£
    ngx_connection_t  *c;
...
{//¶ÁÈ¡FCGIÍ·²¿Êý¾Ý£¬»òÕßproxyÍ·²¿Êý¾Ý¡£
    ...
    if (c->read->timedout) {//¶Á³¬Ê±ÁË£¬ÂÖÑ¯ÏÂÒ»¸ö¡£´íÎóÐÅÏ¢Ó¦¸ÃÒÑ¾­´òÓ¡ÁË
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_TIMEOUT);
        return;
    }
    ...
    if (u->buffer.start == NULL) {//·ÖÅäÒ»¿é»º´æ£¬ÓÃÀ´´æ·Å½ÓÊÜ»ØÀ´µÄÊý¾Ý¡£
    ...
    for ( ;; ) {//²»¶Ïµ÷recv¶ÁÈ¡Êý¾Ý£¬Èç¹ûÃ»ÓÐÁË£¬¾ÍÏÈ·µ»Ø
        n = c->recv(c, u->buffer.last, u->buffer.end - u->buffer.last);
        if (n == NGX_AGAIN) {//»¹Ã»ÓÐ¶ÁÍê£¬»¹ÐèÒª¹Ø×¢Õâ¸öÊÂÇé¡£
        ...
        u->buffer.last += n;//³É¹¦£¬ÒÆ¶¯¶ÁÈ¡µ½µÄ×Ö½ÚÊýÖ¸µ½×îºóÃæ
        rc = u->process_header(r);//ngx_http_fastcgi_process_headerµÈ£¬½øÐÐÊý¾Ý´¦Àí£¬±ÈÈçºó¶Ë·µ»ØµÄÊý¾ÝÍ·²¿½âÎö£¬body¶ÁÈ¡µÈ¡£
        if (rc == NGX_AGAIN) {
            ...
            continue;//¼ÌÐø¡£ÇëÇóµÄHTTPÍ·²¿Ã»ÓÐ´¦ÀíÍê±Ï¡£
        }
        break;//µ½ÕâÀïËµÃ÷ÇëÇóµÄÍ·²¿ÒÑ¾­½âÎöÍê±ÏÁË¡£ÏÂÃæÖ»Ê£ÏÂbodyÁË
    }
    if (rc == NGX_HTTP_UPSTREAM_INVALID_HEADER) {//Í·²¿¸ñÊ½´íÎó¡£³¢ÊÔÏÂÒ»¸ö·þÎñÆ÷¡£
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_INVALID_HEADER);
        return;
    }
    ...
    if (ngx_http_upstream_process_headers(r, u) != NGX_OK) {
        return;//½âÎöÇëÇóµÄÍ·²¿×Ö¶Î¡£Ã¿ÐÐHEADER»Øµ÷Æäcopy_handler£¬È»ºó¿½±´Ò»ÏÂ×´Ì¬ÂëµÈ¡£
    }
    if (!r->subrequest_in_memory) {//Èç¹ûÃ»ÓÐ×ÓÇëÇóÁË£¬ÄÇ¾ÍÖ±½Ó·¢ËÍÏìÓ¦¸ø¿Í»§¶Ë°É¡£
        ngx_http_upstream_send_response(r, u);
        return;
    }
	//Èç¹û»¹ÓÐ×ÓÇëÇóµÄ»°¡£×ÓÇëÇó²»ÊÇ±ê×¼HTTP¡£
    ...
    u->read_event_handler = ngx_http_upstream_process_body_in_memory;//ÉèÖÃbody²¿·ÖµÄ¶ÁÊÂ¼þ»Øµ÷¡£
...
{//½âÎöÇëÇóµÄÍ·²¿×Ö¶Î¡£Ã¿ÐÐHEADER»Øµ÷Æäcopy_handler£¬È»ºó¿½±´Ò»ÏÂ×´Ì¬ÂëµÈ¡£¿½±´Í·²¿×Ö¶Îµ½headers_out
    ngx_str_t                      *uri, args;
    ngx_uint_t                      i, flags;
...
//Èç¹ûÍ·²¿ÖÐÊ¹ÓÃÁËX-Accel-RedirectÌØÐÔ£¬Ò²¾ÍÊÇÏÂÔØÎÄ¼þµÄÌØÐÔ£¬ÔòÔÚÕâÀï½øÐÐÎÄ¼þÏÂÔØ¡££¬ÖØ¶¨Ïò¡£
    ...
    part = &u->headers_in.headers.part;
    h = part->elts;
//´¦ÀíÃ¿Ò»¸öÍ·²¿HEADERÐÐ£¬»Øµ÷Æä¹ÒÔØµÄ¾ä±ú
    for (i = 0; /* void */; i++) {
        if (i >= part->nelts) {
            if (part->next == NULL) {
                break;
        ...
        hh = ngx_hash_find(&umcf->headers_in_hash, h[i].hash, h[i].lowcase_key, h[i].key.len);
        if (hh) {//»Øµ÷ÇëÇóÍ·µÄ¾ä±ú£¬È«²¿×¢²áÔÚÕâ¸öÊý×éÀïÃængx_http_upstream_headers_in.
		...
		///Èç¹ûÃ»ÓÐ×¢²á¾ä±ú£¬ÄÇ¾ÍÀÏÀÏÊµÊµµÄ¿½±´Ò»ÏÂÍ·²¿¾ÍÐÐÁË¡¢
	...
	//¿½±´×´Ì¬ÐÐ£¬ÒòÎªÕâ¸ö²»ÊÇ´æÔÚheaders_inÀïÃæµÄ¡£
    ...
    ngx_http_upstream_t *u)
{
    ...
    for ( ;; ) {
        size = b->end - b->last;//»º³åÇøÌ«Ð¡ÁË¡£Õâ¸ö»º³åÇøÊÇ°üÀ¨ËùÓÐµÄ´Óºó¶Ë·¢¹ýÀ´µÄÊý¾ÝµÄ¡¢
        if (size == 0) {
            ngx_log_error(NGX_LOG_ALERT, c->log, 0, "upstream buffer is too small to read response");
            ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
            return;
        }
        n = c->recv(c, b->last, size);//»¹Òª¶ÁÕâÃ´¶àÊý¾Ý£¬ÊÇ¶¥¶à¶ÁÕâÃ´¶àÊý¾Ý°É?
        if (n == NGX_AGAIN) {//Èç¹ûÃ»ÓÐ¶ÁÈ¡Íê±ÏÄÇÃ´¶à
            break;
        }
        if (n == 0 || n == NGX_ERROR) {
...
{//·¢ËÍÇëÇóÊý¾Ý¸ø¿Í»§¶Ë http://www.pagefault.info/?p=324
    int                        tcp_nodelay;
    ssize_t                    n;
    ngx_int_t                  rc;
    ngx_event_pipe_t          *p;
    ...
    rc = ngx_http_send_header(r);//µ÷ÓÃÃ¿Ò»¸öfilter¹ýÂË£¬´¦ÀíÍ·²¿Êý¾Ý¡£×îºó½«Êý¾Ý·¢ËÍ¸ø¿Í»§¶Ë¡£
    ...
    u->header_sent = 1;//±ê¼ÇÒÑ¾­·¢ËÍÁËÍ·²¿×Ö¶Î¡£
    if (r->request_body && r->request_body->temp_file) {//É¾³ý¿Í»§¶Ë·¢ËÍµÄÊý¾Ý¾ÖÌå
        ngx_pool_run_cleanup_file(r->pool, r->request_body->temp_file->file.fd);
        r->request_body->temp_file->file.fd = NGX_INVALID_FILE;
    }
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
    if (!u->buffering) {
//Õâ¸ö±ê¼ÇµÄº¬Òå¾ÍÊÇnginxÊÇ·ñ»á¾¡¿ÉÄÜ¶àµÄ¶ÁÈ¡upstreamµÄÊý¾Ý¡£Èç¹û¹Ø±Õ£¬Ôò¾ÍÊÇÒ»¸öÍ¬²½µÄ·¢ËÍ£¬
//Ò²¾ÍÊÇ½ÓÊÕ¶àÉÙ£¬·¢ËÍ¸ø¿Í»§¶Ë¶àÉÙ¡£Ä¬ÈÏÕâ¸öÊÇ´ò¿ªµÄ¡£Ò²¾ÍÊÇnginx»ábuf×¡upstream·¢ËÍµÄÊý¾Ý¡£
        if (u->input_filter == NULL) {//Èç¹ûinput_filterÎª¿Õ£¬ÔòÉèÖÃÄ¬ÈÏµÄfilter
            u->input_filter_init = ngx_http_upstream_non_buffered_filter_init;//Êµ¼ÊÉÏÎª¿Õº¯Êý
            u->input_filter = ngx_http_upstream_non_buffered_filter;
            u->input_filter_ctx = r;
        }
		//ÉèÖÃ¶ÁÐ´º¯Êý»Øµ÷
        u->read_event_handler = ngx_http_upstream_process_non_buffered_upstream;
        r->write_event_handler = ngx_http_upstream_process_non_buffered_downstream;//µ÷ÓÃ¹ýÂËÄ£¿éÒ»¸ö¸ö¹ýÂËbody£¬×îÖÕ·¢ËÍ³öÈ¥¡£
        r->limit_rate = 0;
        if (u->input_filter_init(u->input_filter_ctx) == NGX_ERROR) {//µ÷ÓÃinput filter ³õÊ¼»¯º¯Êý
            ngx_http_upstream_finalize_request(r, u, 0);
            return;
        }
        if (clcf->tcp_nodelay && c->tcp_nodelay == NGX_TCP_NODELAY_UNSET) {
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0, "tcp_nodelay");
            tcp_nodelay = 1;//´ò¿ªnodelay£¬×¼±¸½«Êý¾ÝÍêÈ«·¢ËÍ³öÈ¥
        ...
        n = u->buffer.last - u->buffer.pos;//µÃµ½½«Òª·¢ËÍµÄÊý¾ÝµÄ´óÐ¡£¬Ã¿´ÎÓÐ¶àÉÙ¾Í·¢ËÍ¶àÉÙ¡£²»µÈ´ýupstreamÁË
        if (n) {
            u->buffer.last = u->buffer.pos;//×¢ÒâÕâÀï£¬¿ÉÒÔ¿´µ½buffer±»resetÁË¡£
            u->state->response_length += n;//ÉèÖÃ½«Òª·¢ËÍµÄÊý¾Ý´óÐ¡
            if (u->input_filter(u->input_filter_ctx, n) == NGX_ERROR) {//µ÷ÓÃinput filter
                ngx_http_upstream_finalize_request(r, u, 0);
                return;
            }
            ngx_http_upstream_process_non_buffered_downstream(r);//×îÖÕ¿ªÊ¼·¢ËÍÊý¾Ýµ½downstream
        } else {//ËµÃ÷bufferÊÇ¿Õ
            u->buffer.pos = u->buffer.start;
            u->buffer.last = u->buffer.start;
            if (ngx_http_send_special(r, NGX_HTTP_FLUSH) == NGX_ERROR) {//´ËÊ±Ë¢ÐÂÊý¾Ýµ½client
                ngx_http_upstream_finalize_request(r, u, 0);
                return;
            }
            if (u->peer.connection->read->ready) {//Èç¹û¿É¶Á£¬Ôò¼ÌÐø¶ÁÈ¡upstreamµÄÊý¾Ý.
                ngx_http_upstream_process_non_buffered_upstream(r, u);
            }
        }
        return;
    }
//ÏÂÃæ¾ÍÊÇÒª½øÐÐºó¶ËÊý¾Ý»º´æ´¦ÀíµÄ¹ý³ÌÁË¡£Ò²¾ÍÊÇÊ¹ÓÃÁËbuffering±ê¼ÇµÄÌõ¼þÏÂ
    ...
    p->output_ctx = r;
    p->tag = u->output.tag;
    p->bufs = u->conf->bufs;//ÉèÖÃbufs£¬Ëü¾ÍÊÇupstreamÖÐÉèÖÃµÄbufs
...
ngx_http_upstream_process_non_buffered_downstream(ngx_http_request_t *r)
{
    ...
    ngx_http_upstream_process_non_buffered_request(r, 1);//ÒªÁ¢¼´·¢ËÍ£¬ÒòÎªÊÇÒ»¸ö¶ÁÐ´ÊÂ¼þ»Øµ÷º¯Êý¡£
}
...
{//µ÷ÓÃ¹ýÂËÄ£¿é£¬½«Êý¾Ý·¢ËÍ³öÈ¥
    ...
    b = &u->buffer;//ÕÒµ½ÕâÛçÒª·¢ËÍµÄÊý¾Ý
    do_write = do_write || u->length == 0;//do_writeÎª1Ê±±íÊ¾ÒªÁ¢¼´·¢ËÍ¸ø¿Í»§¶Ë¡£
    for ( ;; ) {
        if (do_write) {//ÒªÁ¢¼´·¢ËÍ¡£
            if (u->out_bufs || u->busy_bufs) {
				//Èç¹ûu->out_bufs²»ÎªNULLÔòËµÃ÷ÓÐÐèÒª·¢ËÍµÄÊý¾Ý£¬Èç¹ûu->busy_bufs£¬ÔòËµÃ÷ÉÏ´ÎÓÐÎ´·¢ËÍÍê±ÏµÄÊý¾Ý.
                rc = ngx_http_output_filter(r, u->out_bufs);//Ò»¸ö¸öµ÷ÓÃ¹ýÂËÄ£¿é£¬×îÖÕ·¢ËÍÊý¾Ý¡£
                if (rc == NGX_ERROR) {
                    ngx_http_upstream_finalize_request(r, u, 0);
                    return;
                }
                ngx_chain_update_chains(&u->free_bufs, &u->busy_bufs, &u->out_bufs, u->output.tag);//¸üÐÂbusy chain
            }
            if (u->busy_bufs == NULL) {//ÕâÀïËµÃ÷ÏëÒª·¢ËÍµÄÊý¾Ý¶¼ÒÑ¾­·¢ËÍÍê±Ï
                if (u->length == 0 || upstream->read->eof || upstream->read->error) {
					//´ËÊ±finalize request£¬½áÊøÕâ´ÎÇëÇó
                    ngx_http_upstream_finalize_request(r, u, 0);
                    return;
                }
                b->pos = b->start;//·ñÔòÖØÖÃu->buffer,ÒÔ±ãÓëÏÂ´ÎÊ¹ÓÃ
                b->last = b->start;
            }
        }
        ...
        if (size && upstream->read->ready) {//Èç¹û»¹ÓÐÊý¾ÝÐèÒª½ÓÊÜ£¬²¢ÇÒupstream¿É¶Á£¬Ôò¶ÁÈ¡Êý¾Ý
            n = upstream->recv(upstream, b->last, size);//ÎªÊ²Ã´ÕâÀï»¹ÓÐ¿É¶ÁÊý¾ÝÄØ£¬²»ÊÇÒÑ¾­½Óµ½FCGIµÄ½áÊø°üÁËÂð
            if (n == NGX_AGAIN) {
                break;
            }
            if (n > 0) {//¶ÁÁËÒ»Ð©Êý¾Ý£¬½«Ëü·¢ËÍ³öÈ¥°É£¬Ò²¾ÍÊÇ·ÅÈëµ½out_bufsÁ´±íÀïÃæÈ¥
                u->state->response_length += n;//ÔÙ´Îµ÷ÓÃinput_filter,ÕâÀïÃ»ÓÐreset u->buffer.last,ÕâÊÇÒòÎªÎÒÃÇÕâ¸öÖµ²¢Ã»ÓÐ¸üÐÂ.
                if (u->input_filter(u->input_filter_ctx, n) == NGX_ERROR) {//¾ÍÊÇngx_http_upstream_non_buffered_filter
                    ngx_http_upstream_finalize_request(r, u, 0);
                    return;
                }
            }
            do_write = 1;//ÉèÖÃdo_write,È»ºó·¢ËÍÊý¾Ý.±íÊ¾»¹Òª·¢ËÍÊý¾Ý
            continue;
        }
        break;
    }
	//ÏÂÃæ¾ÍÊÇ¸÷ÖÖ¶ÁÐ´ÊÂ¼þ½á¹¹
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
...
{//½«u->buffer.last - u->buffer.posÖ®¼äµÄÊý¾Ý·Åµ½u->out_bufs·¢ËÍ»º³åÈ¥Á´±íÀïÃæ¡£ÕâÑù¿ÉÐ´µÄÊ±ºò¾Í»á·¢ËÍ¸ø¿Í»§¶Ë¡£
    ...
    cl = ngx_chain_get_free_buf(r->pool, &u->free_bufs);//·ÖÅäÒ»¸ö¿ÕÏÐµÄbuff
    if (cl == NULL) {
        return NGX_ERROR;
    }
    *ll = cl;//½«ÐÂÉêÇëµÄ»º´æÁ´½Ó½øÀ´¡£
    cl->buf->flush = 1;
    cl->buf->memory = 1;
    b = &u->buffer;//È¥³ý½«Òª·¢ËÍµÄÕâ¸öÊý¾Ý£¬Ó¦¸ÃÊÇ¿Í»§¶ËµÄ·µ»ØÊý¾ÝÌå¡£½«Æä·ÅÈë
    cl->buf->pos = b->last;
    b->last += bytes;//ÍùºóÒÆ¶¯
    cl->buf->last = b->last;
    cl->buf->tag = u->output.tag;
//u->length±íÊ¾½«Òª·¢ËÍµÄÊý¾Ý´óÐ¡Èç¹ûÎªNGX_MAX_SIZE_T_VALUE,ÔòËµÃ÷ºó¶ËÐ­Òé²¢Ã»ÓÐÖ¸¶¨ÐèÒª·¢ËÍµÄ´óÐ¡£¬´ËÊ±ÎÒÃÇÖ»ÐèÒª·¢ËÍÎÒÃÇ½ÓÊÕµ½µÄ.
    if (u->length == NGX_MAX_SIZE_T_VALUE) {
        return NGX_OK;
    }
    u->length -= bytes;//¸üÐÂ½«Òª·¢ËÍµÄÊý¾Ý´óÐ¡
    return NGX_OK;
}
...
{//µ±Åöµ½upstream{}Ö¸ÁîµÄÊ±ºòµ÷ÓÃÕâÀï¡£
    ...
    u.host = value[1];
    u.no_resolve = 1;
	//ÏÂÃæ½«u´ú±íµÄÊý¾ÝÉèÖÃµ½umcf->upstreamsÀïÃæÈ¥¡£È»ºó·µ»Ø¶ÔÓ¦µÄupstream{}½á¹¹Êý¾ÝÖ¸Õë¡£
	...
	//ÔÚngx_http_upstream_moduleÄ£¿éÀïÃæ¼ÇÂ¼ÎÒµÄsrv_conf¡£ÀïÃæ¼ÇÂ¼ÁËÎÒÀïÃæÓÐÄÄ¼¸¸öserverÖ¸Áî
    ctx->srv_conf[ngx_http_upstream_module.ctx_index] = uscf;//uscfÀïÃæ¼ÇÂ¼ÁËserverÁÐ±íÐÅÏ¢¡£
    uscf->srv_conf = ctx->srv_conf;//ÕâÒ»Ìõ£¬¼Ç×¡ÎÒÕâ¸öupstreamËùÊôµÄsrv_confÊý×é¡£Ò²¾ÍÊÇËùÊôµÄhttp{}¿éÀïÃæµÄsrv_conf
    ...
    for (m = 0; ngx_modules[m]; m++) {//ÀÏ¹æ¾Ø£¬³õÊ¼»¯ËùÓÐHTTPÄ£¿éµÄsrv,locÅäÖÃ¡£µ÷ÓÃÃ¿¸öÄ£¿éµÄcreate»Øµ÷
    ...
    pcf = *cf;
    cf->ctx = ctx;//ÁÙÊ±ÇÐ»»ctx£¬½øÈëupstream{}¿éÖÐ½øÐÐ½âÎö¡£
...
{//½âÎöµ½"server"µÄÊ±ºòµ÷ÓÃÕâÀï.ÀïÃæÖ»ÊÇÔÚuscf->serversÀïÃæÔö¼ÓÁËÒ»¸öserver,²¢ÉèÖÃºÃngx_http_upstream_server_t½á¹¹µÄÊý¾Ý¡£ÆäËûÃ»¸É¡£
	...
	//upstreamÄ£¿éµÄconfÎªNGX_HTTP_SRV_CONF_OFFSET£¬ËùÒÔ¾ö¶¨ÁËngx_conf_handlerÀïÃæµÄconf²ÎÊý
    time_t                       fail_timeout;
    ngx_str_t                   *value, s;
    ngx_url_t                    u;
    ...
    if (uscf->servers == NULL) {//Èç¹û±¾upstreamµÄserversÊý×éÎª¿Õ£¬³õÊ¼»¯Ö®
        uscf->servers = ngx_array_create(cf->pool, 4, sizeof(ngx_http_upstream_server_t));
        if (uscf->servers == NULL) {
            return NGX_CONF_ERROR;
        }
    }
    us = ngx_array_push(uscf->servers);//Ôö¼ÓÒ»¸öserver.ÏÂÃæÈç¹ûÅäÖÃÊ§°Ü£¬»áÖ±½Ó·µ»ØÊ§°ÜµÄ£¬Ò²¾Í²»ÐèÒª°ÑÕâÏîÉ¾ÁË¡£
    if (us == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    if (ngx_parse_url(cf->pool, &u) != NGX_OK) {//½âÎöÒ»ÏÂURLµÄÊý¾Ý½á¹¹¡£
        if (u.err) {
    ...
    max_fails = 1;
    fail_timeout = 10;
    for (i = 2; i < cf->args->nelts; i++) {//±éÀúºóÃæµÄÃ¿Ò»¸ö²ÎÊý£¬±ÈÈç: server 127.0.0.1:8080 max_fails=3 fail_timeout=30s;
        if (ngx_strncmp(value[i].data, "weight=", 7) == 0) {//µÃµ½ÕûÊýÀàÐÍµÄÈ¨ÖØ¡£
        ...
        if (ngx_strncmp(value[i].data, "max_fails=", 10) == 0) {//½âÎömax_fails²ÎÊý£¬±íÊ¾
        ...
        if (ngx_strncmp(value[i].data, "fail_timeout=", 13) == 0) {//ÕâÃ´fail_timeout¶àµÄÊ±¼äÄÚ£¬³öÏÖmax_failsµÄÊ§°ÜµÄ·þÎñÆ÷½«±»±ê¼ÇÎª³öÎÊÌâµÄ¡£
	...
	//ÏÂÃæ¿½±´ÉèÖÃÒ»ÏÂÕâ¸öserverµÄÏà¹ØÐÅÏ¢¡£
    us->addrs = u.addrs;
    us->naddrs = u.naddrs;
    us->weight = weight;
...
{//Èç¹ûu´ú±íµÄserverÒÑ¾­´æÔÚ£¬Ôò·µ»Ø¾ä±ú£¬·ñÔòÔÚumcf->upstreamsÀïÃæÐÂ¼ÓÒ»¸ö£¬ÉèÖÃ³õÊ¼»¯¡£
//ÔÚngx_http_fastcgi_passµÈÅöµ½ºó¶ËµØÖ·µÄµØ·½£¬»áµ÷ÓÃÕâ¸öº¯Êý£¬Ôö¼ÓÒ»¸öupstreamµÄserver.
//ÕâÀïµÄµ¥Î»ÊÇupstream£¬²»ÊÇserverÐÐ£¬µ÷ÓÃµÄÉÏ²ãÒ»°ãÖ»ÓÐÒ»¸öµØÖ·£¬ÓÚÊÇ¾ÍÖ»ÓÐÒ»¸öserver.µ±×öµ¥¸öupstream´¦Àí
    ngx_uint_t                      i;
    ngx_http_upstream_server_t     *us;
    ngx_http_upstream_srv_conf_t   *uscf, **uscfp;
    ngx_http_upstream_main_conf_t  *umcf;

    if (!(flags & NGX_HTTP_UPSTREAM_CREATE)) {//Èç¹ûÃ»ÓÐÉèÖÃCREATE±êÖ¾£¬±íÊ¾²»ÐèÒª´´½¨¡£
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
		//±éÀúµ±Ç°µÄupstream£¬Èç¹ûÓÐÖØ¸´µÄ£¬Ôò±È½ÏÆäÏà¹ØµÄ×Ö¶Î£¬²¢´òÓ¡ÈÕÖ¾¡£Èç¹ûÕÒµ½ÏàÍ¬µÄ£¬Ôò·µ»Ø¶ÔÓ¦Ö¸Õë¡£
        ...
        return uscfp[i];//ÕÒµ½ÏàÍ¬µÄÅäÖÃÊý¾ÝÁË£¬Ö±½Ó·µ»ØËüµÄÖ¸Õë¡£
    }
	//Ã»ÓÐÕÒµ½ÏàÍ¬µÄÅäÖÃupstream£¬ÏÂÃæ´´½¨Ò»¸ö¡£ÕâÀïµÄsrv_conf¸úserver{}²»ÊÇÒ»»ØÊÂ,ÊÇÖ¸upstream{}ÀïÃæµÄserver xxxx:xxx;ÐÐ
    ...
    if (u->naddrs == 1) {//±ÈÈç: server xx.xx.xx.xx:xx weight=2 max_fails=3;  ¸Õ¿ªÊ¼£¬ngx_http_upstream»áµ÷ÓÃ±¾º¯Êý¡£µ«ÊÇÆänaddres=0.
        uscf->servers = ngx_array_create(cf->pool, 1,  sizeof(ngx_http_upstream_server_t));
        if (uscf->servers == NULL) {
            return NGX_CONF_ERROR;
        }
        us = ngx_array_push(uscf->servers);//¼ÇÂ¼±¾upstream{}¿éµÄËùÓÐserverÖ¸Áî¡£
        if (us == NULL) {
            return NGX_CONF_ERROR;
        }
        ngx_memzero(us, sizeof(ngx_http_upstream_server_t));
        us->addrs = u->addrs;//¿½±´µØÖ· ÐÅÏ¢¡£
        us->naddrs = u->naddrs;
    }
	//
...
{//³õÊ¼»¯upstreamÄ£¿éµÄmain_confÊý¾Ý,ÔÚhttp{}Ö¸Áî½âÎöÍê³ÉÖ®Ç°»áÏÈcreate,Íê³ÉÖ®ºóinitÖ®
//ÏÂÃæÖ»ÊÇ½«ngx_http_upstream_headers_in·ÅÈëumcf->headers_in_hashÀïÃæ¡£
    ...
    uscfp = umcf->upstreams.elts;
    for (i = 0; i < umcf->upstreams.nelts; i++) {//Èç¹ûÅäÖÃÎÄ¼þÀïÃæÃ»ÓÐÖ¸¶¨Ä¬ÈÏ²ßÂÔ£¬ÔòÊ¹ÓÃÂÖÑ¯²ßÂÔ¡£
        init = uscfp[i]->peer.init_upstream ? uscfp[i]->peer.init_upstream : ngx_http_upstream_init_round_robin;
        if (init(cf, uscfp[i]) != NGX_OK) {//µ÷ÓÃÖ®½øÐÐ³õÊ¼»¯¡£Õâ¸öÊÇÖ¸²»Í¬ÀàÐÍµÄÂÖÑ¯²ßÂÔ£¬¹þÏ£²ßÂÔ¶ÔÓ¦µÄ³õÊ¼»¯·½·¨¡£
            return NGX_CONF_ERROR;
        }
    }
