...
ngx_http_access_found(ngx_http_request_t *r, ngx_uint_t deny)
{
    ...
    if (deny) {//是要进行访问权限禁止，那就拒绝掉。
        ...
        if (clcf->satisfy == NGX_HTTP_SATISFY_ALL) {//是否有satisfy控制。
...
ngx_http_access_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->rules == NULL) {//只是简单指一下。子节点指向父节点?
...
ngx_http_access_init(ngx_conf_t *cf)
{
    ...
    cmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_core_module);//得到HTTP模块的核心配置结构。其里面有处理过程的phases结构
    h = ngx_array_push(&cmcf->phases[NGX_HTTP_ACCESS_PHASE].handlers);
    if (h == NULL) {//在NGX_HTTP_ACCESS_PHASE处理过程中增加我这个handler句柄
        return NGX_ERROR;
    }
