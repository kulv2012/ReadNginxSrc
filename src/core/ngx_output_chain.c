...
ngx_int_t
ngx_output_chain(ngx_output_chain_ctx_t *ctx, ngx_chain_t *in)//ctx为&u->output， in为u->request_bufs
{//这里nginx filter的主要逻辑都在这个函数里面,将in参数链表的缓冲块拷贝到ctx->in,然后将ctx->in的数据拷贝到out,然后调用output_filter发送出去。
    off_t         bsize;
    ngx_int_t     rc, last;
    ngx_chain_t  *cl, *out, **last_out;

    if (ctx->in == NULL && ctx->busy == NULL) {//看下面的注释，in,busy是什么?
    ...
    for ( ;; ) {
        ...
        while (ctx->in) {//遍历所有待发送的数据。将他们一个个拷贝到out指向的链表中，为什么要拷贝呢，不知道
            ...
            if (bsize == 0 && !ngx_buf_special(ctx->in->buf)) {//这块内存大小为0，可能有问题。
            ...
            if (ngx_output_chain_as_is(ctx, ctx->in->buf)) { /* move the chain link to the output chain */
                ...
                *last_out = cl;//初始化时，last_out = &out;这里将cl这个头部的节点用out指向。out指向这种处理过的节点头部。
                ...
                cl->next = NULL;//截断这个几点与后面还未处理的节点，然后继续。
                continue;
            }
            if (ctx->buf == NULL) {
            ...
            rc = ngx_output_chain_copy_buf(ctx);//将ctx->in->buf的缓冲拷贝到ctx->buf上面去。
		...
		//out为刚刚处理，拷贝过的缓冲区，下面进行发送了吧应该。调用的是ngx_chain_writer
...
ngx_chain_writer(void *data, ngx_chain_t *in)
{
...
#endif
        size += ngx_buf_size(in->buf);//计算这个节点的数据大小。
	...
	//调用writev将ctx->out的数据全部发送出去。如果没法送完，则返回没法送完毕的部分。记录到out里面
    ctx->out = c->send_chain(c, ctx->out, ctx->limit);//在ngx_event_connect_peer连接上游服务器的时候设置的发送链接函数ngx_send_chain=ngx_writev_chain。
