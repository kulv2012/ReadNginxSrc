...
//将ev挂入queue的头部,queue代表头部，就是全局变量ngx_posted_events或ngx_posted_accept_events。下面需要改全局变量的指向
#define ngx_locked_post_event(ev, queue)                                      \
                                                                              \
    if (ev->prev == NULL) {                                                   \
        ev->next = (ngx_event_t *) *queue;                                    \
        ev->prev = (ngx_event_t **) queue;                                    \
        *queue = ev;//修改链表头的指向，比如ngx_posted_events指向新的，刚加入的节点的位置。\
                                                                              \
        if (ev->next) {                                                       \
            ev->next->prev = &ev->next; //保存旧节点的地址，delete的时候需要这个地址设置为ngx_posted_events.\
        }                                                                     \
