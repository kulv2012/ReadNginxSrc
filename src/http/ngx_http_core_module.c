 ...
 */
    ...
    if (!r->internal && clcf->internal) {//不知道啥意思
    ...
    ngx_http_update_location_config(r);//这个很重要，更新location配置，主要是 r->content_handler = clcf->handler;设置回调。
    ...
    if (rc == NGX_DONE) {//auto redirect,需要进行重定向。
         ...
         */
        if (r->args.len == 0) {//这是啥意思，直接设置为配置的location name 
...
ngx_http_core_content_phase(ngx_http_request_t *r,   ngx_http_phase_handler_t *ph)
{
    ...
    //会调用ngx_http_fastcgi_pass函数，注册本location的处理hander位ngx_http_fastcgi_handler。
...
{//功能:找出指定的URI对应的那个location结构，设置到r->loc_conf数组上面。第一步，查询非精确匹配的static_locations三叉树里面有没有匹配成功的；
...
//找到对应的配置location后，将loc_conf数组首地址设置到r->loc_conf指针上面，这样就切换了location啦。
    ...
    for ( ;; ) {
        ...
        if (len + 1 == (size_t) node->len && node->auto_redirect) {
			//URI长度+1正好等于节点长度，并且呢，有auto_redirect标志，那就可以了，因为支持自动redirect到index.html,不也更好嘛。
