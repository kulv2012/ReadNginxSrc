...
{
	/*ÄÇÃ´£¬µ±ÓÐÇëÇó·ÃÎÊµ½ÌØ¶¨µÄlocationµÄÊ±ºò(¼ÙÉèÕâ¸ölocationÅäÖÃÁËproxy_passÖ¸Áî)£¬
    ...
    ngx_http_set_ctx(r, ctx, ngx_http_proxy_module);//½«Õâ¸öÉÏÏÂÎÄ·Åµ½ÇëÇóµÄctxÊý×éµÄngx_http_proxy_moduleÄ£¿éÖÐ£¬r->ctx[module.ctx_index] = c;
    plcf = ngx_http_get_module_loc_conf(r, ngx_http_proxy_module);//Îª(r)->loc_conf[module.ctx_index]

    u = r->upstream;//ÄÃµ½¸Õ¸ÕÉýÆðµÄupstream
    ...
    u->conf = &plcf->upstream;//»ñÈ¡upstream serverµÄÐÅÏ¢£¬Èç±êÇ©¡¢·þÎñÆ÷ip¡¢portµÈ

#if (NGX_HTTP_CACHE)
    u->create_key = ngx_http_proxy_create_key;
#endif
//Îªupstream×¼±¸¸÷ÖÖÇëÇóµÄ»Øµ÷
    u->create_request = ngx_http_proxy_create_request;//Éú³É·¢ËÍµ½ÉÏÓÎ·þÎñÆ÷µÄÇëÇó»º³å£¨»òÕßÒ»Ìõ»º³åÁ´£©£¬Ò²¾ÍÊÇÒª·¢¸øÉÏÓÎµÄÊý¾Ý
    u->reinit_request = ngx_http_proxy_reinit_request;
    u->process_header = ngx_http_proxy_process_status_line;//´¦Àí»Øµ÷¾ÍÊÇµÚÒ»ÐÐµÄ»Øµ÷£¬µÚÒ»ÐÐ´¦ÀíÍêºó»áÉèÖÃÎªngx_http_proxy_process_header£¬×ßÏÂÒ»²½
    //Ò»¸öupstreamµÄu->read_event_handler ¶ÁÊÂ¼þ»Øµ÷±»ÉèÖÃÎªngx_http_upstream_process_header;£¬Ëû»á²»¶ÏµÄ¶ÁÈ¡Êý¾Ý£¬È»ºóµ÷ÓÃprocess_header
    //¶ÔÓÚFCGI£¬µ±È»ÊÇµ÷ÓÃ¶ÔÓ¦µÄ¶ÁÈ¡FCGI¸ñÊ½µÄº¯ÊýÁË£¬¶ÔÓÚ´úÀíÄ£¿é£¬Ö»Òª´¦ÀíHTTP¸ñÊ½¼´¿É
	...
	//ÉèÖÃÔÚÓÐbufferingµÄ×´Ì¬ÏÂ£¬nginx¶ÁÈ¡upstreamµÄ»Øµ÷£¬Èç¹ûÊÇFCGI£¬ÔòÊÇ¶ÔÓ¦µÄ»Øµ÷ngx_http_fastcgi_input_filterÓÃÀ´½âÎöFCGIÐ­ÒéµÄÊý¾Ý¡£
	//Èç¹ûÎÒÃÇÒªÊµÏÖÎÒÃÇ×Ô¼ºµÄÐ­Òé¸ñÊ½£¬ÄÇ¾ÍÓÃ¶ÔÓ¦µÄ½âÎö·½Ê½¡£
    u->pipe->input_filter = ngx_event_pipe_copy_input_filter;
    u->accel = 1;
	//×¼±¸¶ÁÈ¡ÇëÇóµÄbody²¿·Ö£¬¶ÁÈ¡Íê±ÏºóÁ¢Âíµ÷ÓÃngx_http_upstream_init¿ªÊ¼´¦ÀíÀ²
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    plcf = ngx_http_get_module_loc_conf(r, ngx_http_proxy_module);//µÃµ½ÅäÖÃµÄÊý¾Ý£¬Ò²ÊÇÔÚngx_http_proxy_handlerÀïÃæÉèÖÃµÄ¡£
    if (u->method.len) {
	...
	//ÏÂÃæ²»¶ÏµÄ¶ÔÊý¾Ý³¤¶È½øÐÐÀÛ¼Ó£¬Ëã³öÒª·¢ËÍµ½ºó¶ËµÄÊý¾Ý×Ü¹²ÓÐ¶à³¤£¬´æÔÚlen±äÁ¿ÀïÃæ
    len = method.len + sizeof(ngx_http_proxy_version) - 1 + sizeof(CRLF) - 1;//¼ÓÉÏ" HTTP/1.0"
    escape = 0;
    loc_len = 0;
    unparsed_uri = 0;

    ctx = ngx_http_get_module_ctx(r, ngx_http_proxy_module);//Îª(r)->ctx[module.ctx_index]
    ...
    len += uri_len;//ÀÛ¼ÓµÚÒ»ÐÐ£¬Èç: GET /XXX/Y.html HTTP/1.0
    ngx_http_script_flush_no_cacheable_variables(r, plcf->flushes);//¸ÉÂïµÄ?
    if (plcf->body_set_len) {
        le.ip = plcf->body_set_len->elts;
        le.request = r;
        le.flushed = 1;
        body_len = 0;
		//ÏÂÃæ¼ÆËãÕâÐ©²ÎÊýµÄ¼ÓÈë?
    ...
    if (plcf->upstream.pass_request_headers) {//ÊÇ·ñÒª½«HTTPÇëÇóÍ·²¿µÄHEADER·¢ËÍ¸øºó¶Ë£¬ÒÑHTTP_ÎªÇ°×º
        ...
        header = part->elts;
        for (i = 0; /* void */; i++) {//ÏÂÃæÒ»¸ö¸ö½«¿Í»§¶Ë´«µÝ¹ýÀ´µÄHEADER³¤¶È¼ÆÊý£¬ÐèÒª·¢ËÍ¸øºó¶Ë
            if (i >= part->nelts) {
                if (part->next == NULL) {
                    break;
                }
    ...
    b = ngx_create_temp_buf(r->pool, len);//µ½´ËÎªÖ¹µÄ»°ÎªÇëÇóµÄÍ·²¿µÈ×Ö¶Î£¬body»¹Ã»ÓÐ×¼±¸
    if (b == NULL) {
        return NGX_ERROR;
    }
    cl = ngx_alloc_chain_link(r->pool);//·ÖÅäÒ»¸öÁ´½Ó±í½á¹¹
    if (cl == NULL) {
        return NGX_ERROR;
    }
    cl->buf = b;//ÀÏ°ì·¨£¬½«Õâ¿é»º³å·ÅÈëÁ¬½Ó±íÍ·²¿
	//ÏÂÃæ¿ªÊ¼Ò»¸ö¸ö·±ËöµÄ¿½±´ÁË
    /* the request line */
    ...
    b->last = ngx_cpymem(b->last, ngx_http_proxy_version, sizeof(ngx_http_proxy_version) - 1);//¿½±´HTTP 1.0
    ...
    le.ip = plcf->headers_set_len->elts;
    while (*(uintptr_t *) le.ip) {//Ò»¸ö¸ö¿½±´¼ÓÈëµÄ¶îÍâÍ·²¿×Ö¶Î
        lcode = *(ngx_http_script_len_code_pt *) le.ip;
    ...
    b->last = e.pos;//Ö¸ÕëºóÒÆ


    if (plcf->upstream.pass_request_headers) {//ÕâÀïÒ»¸ö¸ö¿½±´¿Í»§¶Ë·¢ËÍ¹ýÀ´µÄÍ·²¿×Ö¶Î
        part = &r->headers_in.headers.part;
        header = part->elts;
        for (i = 0; /* void */; i++) {
            if (i >= part->nelts) {
    ...
    }//¿Í»§¶Ë·¢ËÍµÄÍ·²¿¿½±´Íê±Ï¡£ÏÂÃæÔö¼Ó2¸ö¿ÕÐÐ´ú±íÍ·²¿Íê±Ï£¬×¼±¸¿½±´°üÌå
    /* add "\r\n" at the header end */
    *b->last++ = CR; *b->last++ = LF;

    if (plcf->body_set) {//ÕâÀïÊÇÖ¸¶îÍâÔö¼ÓµÄbodyÊý¾ÝÂð,»¹ÊÇËµbody²¿·ÖÓÉ½Å±¾ÖØÐ´£¬²»ÐèÒª¿Í»§¶ËµÄbody å?
    ...
    if (plcf->body_set == NULL && plcf->upstream.pass_request_body) {//Èç¹ûÐèÒª´«µÝbody£¬Ò»°ã¿Ï¶¨ÒªµÄ
        body = u->request_bufs;//¿Í»§¶Ë·¢ËÍ¹ýÀ´µÄÊý¾Ý£¬body£¬Õâ¸ö½á¹¹ÎªÒ»¸öÁ¬½Ó±í
        u->request_bufs = cl;//Ö¸ÏòÐÂµÄ·ÖÅäµÄÁ¬½Ó±í£¬ÄÇÕâÀï±íÊ¾µÄµ±È»¾ÍÊÇÒª·¢ËÍ¸øºó¶ËµÄÊý¾ÝÁË
		//ÏÂÃæ¶ÔÃ¿Ò»¿éÁ¬½Ó±í½á¹¹£¬¶¼»á¿½±´µÄ£¬ÄÇÎÊÌâÊÇ£¬Õâ¸örequest_bufsÀïÃæÓ¦¸ÃÃ»ÓÐÍ·²¿Êý¾Ý°É
        while (body) {//²»¶ÏµÄ±éÀúÕâ¸öÁ¬½Ó±í½á¹¹µÄÃ¿Ò»¿é»º³åÇø
            b = ngx_alloc_buf(r->pool);//³¡¾°Ò»¸ö»º³åÇø£¬Ã¿´Î
            if (b == NULL) {
                return NGX_ERROR;
            }
            ngx_memcpy(b, body->buf, sizeof(ngx_buf_t));
            cl->next = ngx_alloc_chain_link(r->pool);//´´½¨Ò»¸öÐÂµÄ Á¬½Ó±í£¬Á¬½Óµ½ºóÃæ
            if (cl->next == NULL) {
                return NGX_ERROR;
            }
            cl = cl->next;//ÍøºóÒÆ¶¯
            cl->buf = b;//Á¬½Óµ½½ÚµãÉÏ
            body = body->next;//¼ÌÐøÏÂÒ»¿é
        }
        b->flush = 1;
    } else {
        u->request_bufs = cl;
    }
    cl->next = NULL;//»º³åÇøÁ´±í½áÎ²£¬×¼±¸Íê±Ï
//ÖÁ´Ë£¬Òª·¢ËÍ¸ø¿Í»§¶ËµÄÊý¾ÝÔÚÕâÀïÒÑ¾­·ÖÅäÍê±Ï£¬ÄÇÃ´£¬ÆäÊµÎÒÃÇÍêÈ«¿ÉÒÔÔÚÇ°Ãæ¼ÓÉÏnsheadÍ·²¿¡£ÕâÑùUB¾ÍÄÜ·½±ãµÄ½ÓÍê
    return NGX_OK;
}
...
ngx_http_proxy_process_status_line(ngx_http_request_t *r)
{
    ...
    u = r->upstream;
    rc = ngx_http_parse_status_line(r, &u->buffer, &ctx->status);//½âÎö×´Ì¬ÐÐ£¬Ò²¾ÍÊÇµÚÒ»ÐÐ
    if (rc == NGX_AGAIN) {
        return rc;//Èç¹ûÊý¾Ý»¹Ã»ÓÐÄÇÃ´¶à
    }
    if (rc == NGX_ERROR) {
#if (NGX_HTTP_CACHE)
        if (r->cache) {
    ...
    if (u->state) {//ÓÐ×´Ì¬×Ö¶Î£¬¾Í±£´æ×´Ì¬Âë
        u->state->status = ctx->status.code;
    }
    u->headers_in.status_n = ctx->status.code;//±£´æµ½headers_inÀïÃæ
    ...
    u->process_header = ngx_http_proxy_process_header;//ÏÂÃæ½øÐÐHTTP Í·²¿µÄ½âÎö¡£Èç¹ûÃ»ÓÐÍê±Ïµ±È»ÏÈ·µ»ØÁË¡£
...
{//ÉÏÓÎÄ£¿éµÄ¿É¶ÁÊÂ¼þ»Øµ÷»áµ÷ÓÃngx_http_upstream_process_header£¬È»ºóµ÷ÓÃprocess_header½øÐÐÍ·²¿Êý¾ÝµÄ½âÎöÁË¡£
//×¢ÒâÕâÀïÖ»¶ÁÈ¡Í·²¿×Ö¶Î£¬Ã»ÓÐ¶ÁÈ¡body²¿·Ö£¬ÄÇbody²¿·ÖÔÙÉêÃ÷µØ·½¶ÁÈ¡µÄÄØ,Èç:
//²»¹ÜbufferingÊÇ·ñ´ò¿ª£¬ºó¶Ë·¢ËÍµÄÍ·¶¼²»»á±»buffer£¬Ê×ÏÈ»á·¢ËÍheader£¬È»ºó²ÅÊÇbodyµÄ·¢ËÍ£¬¶øbodyµÄ·¢ËÍ¾ÍÐèÒªÇø·ÖbufferingÑ¡ÏîÁË¡£
    ngx_int_t                       rc;
    ngx_table_elt_t                *h;
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {//ÏÂÃæ½«Õâ¸öÐÂµÄÍ·²¿×Ö¶Î±£´æÆðÀ´
            /* a header line has been parsed successfully */
            h = ngx_list_push(&r->upstream->headers_in.headers);//È¡Ò»¸öÔªËØ
            if (h == NULL) {
                return NGX_ERROR;
            }
            ...
            if (hh && hh->handler(r, h, hh->offset) != NGX_OK) {//¶Ô×Ö¶Î½øÐÐ´¦Àí¡£
                return NGX_ERROR;
            }
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,  "http proxy header: \"%V: %V\"",  &h->key, &h->value);
            continue;
        }
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {//È«²¿¶¼ÒÑ¾­½âÎöÍê±ÏÁË£¬¿Ï¶¨ÊÇÅöµ½\R\NÁË£¬GOOD
        ...
        if (rc == NGX_AGAIN) {//»¹Ã»ÓÐÍê±Ï£¬ÐèÒª¼ÌÐø¼àÌý¶Á¡£
            return NGX_AGAIN;
        }
    ...
    ngx_http_proxy_loc_conf_t *prev)
{
    ...
    h = conf->upstream.cache ? ngx_http_proxy_cache_headers: ngx_http_proxy_headers;
#else
...
{//Õâ¸öº¯Êý»áÔÚnginxÅöµ½proxy_passÖ¸ÁîµÄÊ±ºò£¬¾Íµ÷ÓÃ£¬È»ºóÉèÖÃÏà¹ØµÄ»Øµ÷º¯Êýµ½¶ÔÓ¦µÄÄ£¿éÖÐÈ¥
    ...
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
	////ÉèÖÃlocµÄhandler£¬Õâ¸öclcf->handler»áÔÚngx_http_update_location_config()ÀïÃæ¸³Óèr->content_handler£¬
	//´Ó¶øÔÚNGX_HTTP_CONTENT_PHASEÀïÃæµ÷ÓÃÕâ¸öhandler£¬¼´ngx_http_proxy_handler¡£
    clcf->handler = ngx_http_proxy_handler;
    ...
    u.url.len = url->len - add;//ÉèÖÃurlµÄ³¤¶È£¬³ýÈ¥http://(https://)
    u.url.data = url->data + add;//ÉèÖÃurl£¬³ýÈ¥http://(https://)£¬±ÈÈçÔ­À´ÊÇ"http://backend1"£¬ÏÖÔÚ¾ÍÊÇ"backend1"
    u.default_port = port;
    u.uri_part = 1;
    u.no_resolve = 1;

    plcf->upstream.upstream = ngx_http_upstream_add(cf, &u, 0);//ÕÒµ½Ö¸¶¨Ãû×ÖµÄupstreamµÄ×é
