...
//ÕâÀï¸ºÔğÁ¬½Óºó¶Ë·şÎñ£¬È»ºóÉèÖÃ¸÷¸ö¶ÁĞ´ÊÂ¼ş»Øµ÷¡£×îºóÈç¹ûÁ¬½Ó½¨Á¢³É¹¦£¬»áµ÷ÓÃngx_http_upstream_send_request½øĞĞÊı¾İ·¢ËÍ¡£
    ...
    {//request_bodyÊÇFCGI½á¹¹µÄÊı¾İ¡£Èç¹û¿Í»§¶Ë·¢ËÍÁËBODY£¬²¢ÇÒÕâ¸öBODY´æÔÚÁÙÊ±ÎÄ¼şÀïÃæ£¬ÄÇÃ´ÎÒÃÇ¿ÉÒÔÁ¢¼´¸´ÓÃ r->request_body->bufÁË¡£
        /*
...
{//µ÷ÓÃÊä³öµÄ¹ıÂËÆ÷£¬·¢ËÍÊı¾İµ½ºó¶Ë
	...
	//ngx_http_proxy_create_request½«¿Í»§¶Ë·¢ËÍµÄÊı¾İ¿½±´µ½ÕâÀï£¬Èç¹ûÊÇ´Ó¶ÁĞ´ÊÂ¼ş»Øµ÷½øÈëµÄ£¬ÔòÕâÀïµÄrequest_sentÓ¦¸ÃÎª1£¬
...
{//µ±Åöµ½upstream{}Ö¸ÁîµÄÊ±ºòµ÷ÓÃÕâÀï¡£
    ...
    value = cf->args->elts;
    u.host = value[1];//°ÑµÚÒ»¸ö²ÎÊıµ±×öhost£¬ÆäÊµ¿ÉÒÔÊÇÈÎºÎ×Ö·û´®±íÊ¾¡£
    u.no_resolve = 1;
...
//Ôö¼ÓÒ»¸öserver.ÏÂÃæÈç¹ûÅäÖÃÊ§°Ü£¬»áÖ±½Ó·µ»ØÊ§°ÜµÄ£¬Ò²¾Í²»ĞèÒª°ÑÕâÏîÉ¾ÁË¡£
    if (us == NULL) {
        return NGX_CONF_ERROR;
    }
...
//±éÀúºóÃæµÄÃ¿Ò»¸ö²ÎÊı£¬±ÈÈç: server 127.0.0.1:8080 max_fails=3 fail_timeout=30s;
	...
	//ÕâÃ´fail_timeout¶àµÄÊ±¼äÄÚ£¬³öÏÖmax_failsµÄÊ§°ÜµÄ·şÎñÆ÷½«±»±ê¼ÇÎª³öÎÊÌâµÄ¡£
...
//ÕâÀïµÄµ¥Î»ÊÇupstream£¬²»ÊÇserverĞĞ£¬µ÷ÓÃµÄÉÏ²ãÒ»°ãÖ»ÓĞÒ»¸öµØÖ·£¬ÓÚÊÇ¾ÍÖ»ÓĞÒ»¸öserver.µ±×öµ¥¸öupstream´¦Àí
    ...
    if (!(flags & NGX_HTTP_UPSTREAM_CREATE)) {//Èç¹ûÃ»ÓĞÉèÖÃCREATE±êÖ¾£¬±íÊ¾²»ĞèÒª´´½¨¡£
        if (ngx_parse_url(cf->pool, u) != NGX_OK) {//¼òÎöÒ»ÏÂµØÖ·¸ñÊ½£¬Ãû³Æ£¬unix:Óò£¬inet6,4µØÖ·£¬http://host:port/µÈ
            if (u->err) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,  "%s in upstream \"%V\"", u->err, &u->url);
            }
            return NULL;
    ...
    uscf->flags = flags;
    uscf->host = u->host;//ÕâÀïµÄhostÆäÊµ¿ÉÒÔÎªÈÎºÎ×Ö·û´®¶¼ĞĞµÄ¡£
    uscf->file_name = cf->conf_file->file.name.data;
    uscf->line = cf->conf_file->line;
    uscf->port = u->port;//Ã»ÓĞ¾ÍÄ¬ÈÏ80
