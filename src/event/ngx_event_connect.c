...
{//»ñÈ¡Ò»¸ö¿ÉÓÃµÄpeer,È»ºóÁ¬½ÓËü.²¢×¢²á¿É¶Á£¬¿ÉÐ´ÊÂ¼þ
    int                rc;
    ngx_int_t          event;
    ngx_err_t          err;
    ngx_uint_t         level;
    ...
    s = ngx_socket(pc->sockaddr->sa_family, SOCK_STREAM, 0);//ÐÂ½¨Ò»¸öSOCK
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, pc->log, 0, "socket %d", s);
    if (s == -1) {
        ngx_log_error(NGX_LOG_ALERT, pc->log, ngx_socket_errno, ngx_socket_n " failed");
        return NGX_ERROR;
    }
    c = ngx_get_connection(s, pc->log);//ÔÚngx_cycle->free_connectionsÀïÃæÕÒÒ»¸ö¿ÕÏÐµÄÎ»ÖÃ´æ·ÅÕâ¸öÁ¬½Ó£¬È»ºó³õÊ¼»¯Ïà¹Ø³ÉÔ±
    if (c == NULL) {//µÃµ½Ò»¸öÁ¬½Ó½á¹¹¡£
    ...
    rev = c->read;//ÉèÖÃÕâ¸öÁ¬½ÓµÄ¶ÁÐ´ÊÂ¼þ½á¹¹
    wev = c->write;
    ...
    if (ngx_add_conn) {//µ÷ÓÃngx_epoll_add_connection×¢²áepoolµÄ¸÷¸öÊÂ¼þ£¬²»¹ý¶ÁÐ´ÊÂ¼þ½á¹¹´ý»áÉèÖÃ¡£
        if (ngx_add_conn(c) == NGX_ERROR) {
            goto failed;
        }
    }
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, pc->log, 0, "connect to %V, fd:%d #%d", pc->name, s, c->number);
    rc = connect(s, pc->sockaddr, pc->socklen);//È»ºóÁ¬½ÓËü
    ...
    if (ngx_add_event(rev, NGX_READ_EVENT, event) != NGX_OK) {//ÉèÖÃ¶ÁÊÂ¼þ
        goto failed;
    }
    if (rc == -1) {
        /* NGX_EINPROGRESS */
        if (ngx_add_event(wev, NGX_WRITE_EVENT, event) != NGX_OK) {//ÉèÖÃÐ´ÊÂ¼þ
            goto failed;
        }
        return NGX_AGAIN;
    }
