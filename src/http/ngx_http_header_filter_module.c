...
{//最后一个头部处理模块。申请缓冲区，将各个头部字段都拷贝进去，然后调用ngx_http_write_filter写头部字段到客户端
    u_char                    *p;
    size_t                     len;
    ...
    if (r->header_sent) {//如果已经调用过这里了，就直接返回咯。
        return NGX_OK;
    }
    r->header_sent = 1;//此函数只用调用一次，每个请求。
    if (r != r->main) {
        return NGX_OK;
    }
    ...
    if (r->headers_out.last_modified_time != -1) {//如果设置了上次时间更改时间的返回头部。
        if (r->headers_out.status != NGX_HTTP_OK && r->headers_out.status != NGX_HTTP_PARTIAL_CONTENT
            && r->headers_out.status != NGX_HTTP_NOT_MODIFIED)
        {//如果不是200，且不是不发内容206，和资源未更改头304.则清空last_modified_time
	...
	//下面一个个计算输出的头部需要多少字节。
    len = sizeof("HTTP/1.x ") - 1 + sizeof(CRLF) - 1 + sizeof(CRLF) - 1;///* the end of the header */
    /* status line */
    if (r->headers_out.status_line.len) {//如果有状态行，就加上去。
        len += r->headers_out.status_line.len;
        status_line = &r->headers_out.status_line;
#if (NGX_SUPPRESS_WARN)
        status = 0;
#endif
    } else {//如果后端么有设置错误码字段，我们就需要根据错误码设置一个默认的错误信息字段。
        ...
        {/* 5XX */
            status = status - NGX_HTTP_INTERNAL_SERVER_ERROR + NGX_HTTP_LEVEL_200 + NGX_HTTP_LEVEL_300n+ NGX_HTTP_LEVEL_400;
            status_line = &ngx_http_status_lines[status];
            len += ngx_http_status_lines[status].len;
        } else {
            len += NGX_INT_T_LEN;
    ...
    if (r->headers_out.server == NULL) {//设置server字段
        len += clcf->server_tokens ? sizeof(ngx_http_server_full_string) - 1: sizeof(ngx_http_server_string) - 1;
    }
    if (r->headers_out.date == NULL) {//如果没有设置时间，默认添加一个
        len += sizeof("Date: Mon, 28 Sep 1970 06:00:00 GMT" CRLF) - 1;
    }
    if (r->headers_out.content_type.len) {//有内容类型，则需要设置
    ...
    header = part->elts;
    for (i = 0; /* void */; i++) {//遍历每一个输出头部字段，计算其所需要花费的长度。
        if (i >= part->nelts) {
            if (part->next == NULL) {
                break;
            }
    ...
    b = ngx_create_temp_buf(r->pool, len);//创建那么多缓冲区。
    if (b == NULL) {
        return NGX_ERROR;
    }
//下面开始一个个头部的拷贝到临时缓冲区里面。
    /* "HTTP/1.x " */
    ...
    *b->last++ = CR; *b->last++ = LF;
    ...
    part = &r->headers_out.headers.part;
    header = part->elts;
    for (i = 0; /* void */; i++) {//拷贝每一个从后端传过来的头部字段。
        if (i >= part->nelts) {
            if (part->next == NULL) {
                break;
            }
    ...
    *b->last++ = CR; *b->last++ = LF;
    r->header_size = b->last - b->pos;//记录一下返回的头部字段的长度
    if (r->header_only) {
        b->last_buf = 1;
    }
    out.buf = b;//记录这坨数据的内存
...
{
    ngx_http_top_header_filter = ngx_http_header_filter;//我是最后一个头部过滤模块，所以不需要保存前面一个。
