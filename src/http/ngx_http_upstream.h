...
struct ngx_http_upstream_srv_conf_s {//Ò»¸öupstream{}ÅäÖÃ½á¹¹µÄÊý¾Ý,Õâ¸öÊÇumcf->upstreamsÀïÃæµÄÊý×éÏî¡£umcfÊÇupstreamÄ£¿éµÄ¶¥²ãÅäÖÃÁË¡£
    ...
    ngx_uint_t                       flags;
    ngx_str_t                        host;//±íÊ¾ÕâÌõupstream backend{}µÄbackend£¬°ÑËüµ±×öhost£¬ÆäÊµ¿ÉÒÔÎªÈÎºÎ×Ö·û´®¶¼ÐÐµÄ¡£
    u_char                          *file_name;//ÅäÖÃÎÄ¼þÃû³Æ
    ngx_uint_t                       line;//ÅäÖÃÎÄ¼þÖÐµÄÐÐºÅ
    in_port_t                        port;//ÉÏÃæ¸úhost¶ÔÓ¦£¬±íÊ¾¶Ë¿Ú¡£
...
typedef struct {
    ...
    ngx_addr_t                      *local;//ÓÃÀ´°ó¶¨±¾µØIPµÄ£¬±ÈÈçÏÞÖÆÖ»´ÓÖ¸¶¨Íø¿¨·¢ËÍÊý¾Ý¸ø¶Ô¶Ë¡£
...
struct ngx_http_upstream_s {//±¾½á¹¹ÌåÓÃÀ´±£´æÒ»¸öÁ¬½ÓµÄupstreamÐÅÏ¢£¬°üÀ¨¸÷ÖÖÐèÒªupstream»Øµ÷µÄº¯ÊýµÈ¡£
    ...
    //µ÷ÓÃngx_output_chainºó£¬Òª·¢ËÍµÄÊý¾Ý¶¼»á·ÅÔÚÕâÀï£¬È»ºó·¢ËÍ£¬È»ºó¸üÐÂÕâ¸öÁ´±í£¬Ö¸ÏòÊ£ÏÂµÄ»¹Ã»ÓÐµ÷ÓÃwritev·¢ËÍµÄ¡£
    ...
    ngx_buf_t                        buffer;///¶ÁÈ¡ÉÏÓÎ·µ»ØµÄÊý¾ÝµÄ»º³åÇø£¬Ò²¾ÍÊÇproxy£¬FCGI·µ»ØµÄÊý¾Ý¡£ÕâÀïÃæÓÐhttpÍ·²¿£¬Ò²¿ÉÄÜÓÐbody²¿·Ö¡£
    									//Æäbody²¿·Ö»á¸úevent_pipe_tµÄpreread_bufs½á¹¹¶ÔÓ¦ÆðÀ´¡£¾ÍÊÇÔ¤¶ÁµÄbuf£¬ÆäÊµÊÇi²»Ð¡ÐÄ¶Áµ½µÄ¡£
    size_t                           length;//Òª·¢ËÍ¸ø¿Í»§¶ËµÄÊý¾Ý´óÐ¡£¬»¹ÐèÒª¶ÁÈ¡ÕâÃ´¶à½øÀ´¡£
    ...
    ngx_int_t                      (*input_filter)(void *data, ssize_t bytes);//ÓÃÀ´¶ÁÈ¡ºó¶ËµÄÊý¾Ý£¬·ÇbufferingÄ£Ê½¡£
    										//ngx_http_upstream_non_buffered_filter£¬ngx_http_memcached_filterµÈ¡£
    										//Õâ¸öº¯ÊýµÄµ÷ÓÃÊ±»ú: ngx_http_upstream_process_non_buffered_upstreamµÈµ÷ÓÃngx_unix_recv½ÓÊÕµ½upstream·µ»ØµÄÊý¾Ýºó
    										//¾Íµ÷ÓÃÕâÀï½øÐÐÐ­Òé×ª»»£¬²»¹ýÄ¿Ç°×ª»»²»¶à¡£
    ...
    ngx_str_t                        schema;//¾ÍÊÇÇ°ÃæµÄhttp,https,mecached://µÈ¡£
    ngx_str_t                        uri;
...
#endif
    unsigned                         buffering:1;//ÊÇ·ñÒªbuffer ºó¶ËµÄÊý¾Ý£¬Èç¹ûÒªÓÃevent_pipeµÄ·½Ê½·¢ËÍÊý¾Ý
